...
var TableDatatablesAjax = function () {
    ...
    var handleRecords = function () {
            ...
            dataTable: {// here you can define a typical datatable settings from http://datatables.net/usage/options
                ...
                "columns": [{
                    ...
                    },
                    {
                     ...
                     {
                        "data": 6,
                        className: 'text-center last_td_action',
                        "bSortable": false
                    }],
                "columnDefs": [{
                        "targets": [6],
                ...
                "ajax": {
                    "url": window.site_url + "/powerpanel/maintenance/get_list", // ajax source
                },
        ...
        $(document).on('keyup', '#searchfilter', function (e) {
            ...
            if (action.length >= 2) {
                $.cookie('maintenancesearch', action);
                grid.setAjaxParam("customActionType", "group_action");
                grid.setAjaxParam("searchValue", action);
                grid.setAjaxParam("id", grid.getSelectedRows());
                grid.getDataTable().ajax.reload();
            } else {
                $.removeCookie('maintenancesearch');
                grid.setAjaxParam("customActionType", "group_action");
                grid.setAjaxParam("searchValue", action);
                grid.setAjaxParam("id", grid.getSelectedRows());
                grid.getDataTable().ajax.reload();
        ...
        if (!showChecker) {
            grid.getDataTable().column(2).visible(false);
            grid.getDataTable().column(3).visible(false);
        } else {
            if ($.cookie('Maintenance_publish_P_3') == 'Y') {
            ...
            dataTable: {// here you can define a typical datatable settings from http://datatables.net/usage/options
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "data": 3,
                        className: 'text-right last_td_action',
                        "bSortable": false
                    }],
                "ajax": {
                    "url": window.site_url + "/powerpanel/maintenance/get_list_New", // ajax source
                },
                'fnCreatedRow': function (nRow, aData, iDataIndex) {
...
$(window).on('load', function () {
    if ($.cookie('maintenancesearch')) {
        $('#searchfilter').val($.cookie('maintenancesearch'));
...
function reorder(curOrder, excOrder) {
    var ajaxurl = site_url + '/powerpanel/maintenance/reorder';
    $.ajax({
        url: ajaxurl,
        data: {
            order: curOrder,
