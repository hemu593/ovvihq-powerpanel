...
var Custom = function() {
    return {
        ...
        checkVersion: function() {
            $('.imguploader').addClass('hide');
            var radioValue = $("input[name='chrRenewal']:checked").val();

            if (radioValue == 'on') {
                $('.imguploader').addClass('show');
            } else {
                $('.imguploader').removeClass('show');
            }
    ...
    var handleTeam = function() {
        $("#frmLicenceRegister").validate({
            ...
            messages: {
                ...
                company_id: {
                    required: "Company Id field is required."
                },
                cperson: {
                    required: "Contact Person field is required."
                },
                start_date: {
                    required: "Date of Issue field is required."
                },
                address: {
                ...
                'service[]': {
                    required: "Please select network/service."
                },
                order: {
            ...
            invalidHandler: function(event, validator) { //display error alert on form submit 
                ...
                $('.alert-danger', $('#frmLicenceRegister')).show();
            },
        ...
        $('#frmLicenceRegister input').on('keypress', function(e) {
            if (e.which == 13) {
                if ($('#frmLicenceRegister').validate().form()) {
                    $('#frmLicenceRegister').submit(); //form validation success, call ajax form submit
                    $("button[type='submit']").attr('disabled', 'disabled');
                }
                return false;
            }
...
jQuery(document).ready(function() {
    ...
    $("input[id^='start_date']").rules('add', {
        required: true
    });

    $("input[id^='renewal_date']").datepicker({
        autoclose: true,
        minuteStep: 5,
        startdate: true,
        format: 'yyyy-mm-dd'
