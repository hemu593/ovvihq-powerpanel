...
var TableDatatablesAjax = function () {
    ...
    var handleRecords = function () {
        grid = new Datatable();
        if(!tableState){
              $.removeCookie('projectCategorySearch');
              $("#datatable_ajax").DataTable().state.clear();
              $("#datatable_ajax").DataTable().destroy();
            }
        if (tableState) {
              if($.cookie('projectCategorySearch')) {
                var projectCategorySearch = $.cookie('projectCategorySearch');
                $('#searchfilter').val(projectCategorySearch);
                grid.setAjaxParam("searchValue", projectCategorySearch);
        ...
        $(document).on('keyup', '#searchfilter', function (e) {
            ...
            if (action.length >= 2) {
                $.cookie('projectCategorySearch',action);
                grid.setAjaxParam("customActionType", "group_action");
