...
var TableDatatablesAjax = function() {
		...
		var handleRecords = function() {
				...
				grid = new Datatable(); //productSearch
				if(!tableState){
				  $.removeCookie('productSearch');
		      $("#datatable_ajax").DataTable().state.clear();
		      $("#datatable_ajax").DataTable().destroy();
		    }
		    if (tableState) {
		    	  if($.cookie('productSearch')) {
					      var productSearch = $.cookie('productSearch');
	                $('#searchfilter').val(productSearch);
	                grid.setAjaxParam("searchValue", productSearch);
