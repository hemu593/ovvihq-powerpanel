...
var TableDatatablesAjax = function() {
		...
		var handleRecords = function() {
				grid = new Datatable(); //ClientCategorySearch
			 if(!tableState){
				  $.removeCookie('ClientCategorySearch');
		      $("#datatable_ajax").DataTable().state.clear();
		      $("#datatable_ajax").DataTable().destroy();
		    }
		    if (tableState) {
		    	  if($.cookie('ClientCategorySearch')) {
					      var ClientCategorySearch = $.cookie('ClientCategorySearch');
	                $('#searchfilter').val(ClientCategorySearch);
	                grid.setAjaxParam("searchValue", ClientCategorySearch);
				...
				grid.init({
						...
						dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
								...
								"ajax": {
										"url": window.site_url + "/powerpanel/client-category/get_list", // ajax source
								},
				...
				$(document).on('keyup', '#searchfilter', function(e) {
						...
						if (action.length >= 2) {
								$.cookie('ClientCategorySearch',action);				
								grid.setAjaxParam("customActionType", "group_action");
...
function reorder(curOrder, excOrder) {
		var ajaxurl = site_url + '/powerpanel/client-category/reorder';
		$.ajax({
				url: ajaxurl,
				data: {
						order: curOrder,
