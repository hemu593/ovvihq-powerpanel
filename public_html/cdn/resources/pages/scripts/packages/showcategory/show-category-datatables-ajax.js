...
var TableDatatablesAjax = function() {
		...
		var handleRecords = function() {
				grid = new Datatable(); //showCategorySearch
				if(!tableState){
				  $.removeCookie('showCategorySearch');
		      $("#datatable_ajax").DataTable().state.clear();
		      $("#datatable_ajax").DataTable().destroy();
		    }
		    if (tableState) {
		    	  if($.cookie('showCategorySearch')) {
					      var showCategorySearch = $.cookie('showCategorySearch');
	                $('#searchfilter').val(showCategorySearch);
	                grid.setAjaxParam("searchValue", showCategorySearch);
				...
				$(document).on('keyup', '#searchfilter', function(e) {
						...
						if (action.length >= 2) {
								$.cookie('showCategorySearch',action);
								grid.setAjaxParam("customActionType", "group_action");
