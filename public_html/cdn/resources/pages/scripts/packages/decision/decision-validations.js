...
var Custom = function() {
    return {
        ...
        checkType: function() {
            var radioValue = $("input[name='field_type']:checked").val();
            if (radioValue == 'link') {
                $('#DocId').hide();

                $('#linkId').show();

            } else if (radioValue == 'document') {
                $('#DocId').show();
                $('#linkId').hide();
        ...
        getModuleRecords: function(sectorName) {
            ...
            var ajaxUrl = site_url + '/powerpanel/decision/getCategory';
            jQuery.ajax({
                type: "POST",
                url: ajaxUrl,
                dataType: 'HTML',
        ...
        $("#frmDecision").validate({
            ...
            rules: {
                ...
                link: {
                    required: {
                        depends: function() {
                            return $('#link_type[value="link"]:checked').length > 0;
                        }
                    },
                    url: true
                },
                doc_id: {
                    required: {
                        depends: function() {
                            return $('#document_type[value="document"]:checked').length > 0;
                        }
                    },
                },
                start_date_time: {
...
jQuery(document).ready(function() {
    ...
    Custom.init();
    Custom.checkType();
    $(document).on('click', '.fieldtype', function(e) {
        Custom.checkType();
    });
