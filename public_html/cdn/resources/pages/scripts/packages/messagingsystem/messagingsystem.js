$(document).ready(function () {
    ...
    $("#varShortDescription").keypress(function (e) {
        if (e.which == 13) {
...
function JumpIntoUser(id)
{
    ...
    $('.chat-user-list li[data-userid=' + id + ']').addClass('active');
    $("#replayform").show();
    $("#toid").val(id);
    getmessage(id);
}
...
//     });
// });
$(document).on('click', 'ul.chat-user-list li', function () {
    $('ul.chat-user-list li').removeClass("active");
...
function getmessage(userid) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            let msgCount = $('.msg-number-count').length;
            if(msgCount <= 1) {
                $('.pulse-danger').remove();
            }
            // $(".mcscroll").mCustomScrollbar({
...
//            success: function(data) {
//            if (data != 0) {
//            $('.chat-user-list li[data-userid=' + data + ']').addClass('active');
//            getmessage(data)
//                    $("#replayform").show();
//            $("#toid").val(data);
//            } else {
...
$(document).ready(function () {
    $(document).on('click', '#btnSubmit', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                var data = data.split('@@');
                $('ul.chat-user-list').html(data[1]);
                $('ul.chat-user-list li[data-userid=' + data[0] + ']').addClass('active');
                getmessage(data[0]);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (($("#formtype").val() == 'edit')) {
                    ...
                    $('#html_chat_dis_' + data1[0]).before('<i class="fa fa-pencil" style="" title="This message has been edited."></i>');
...
$(document).ready(function () {
    $(document).on('click', '#refersh', function () {
        $(".chat-user-list li.active").each(function (index, value) {
...
function refreshauto() {
    ...
    var i = 0;
    var activeuserid = '';
    $(".chat-user-list li").each(function (index, value) {
    ...
    $.ajax({
        ...
        success: function (datacount) {
            ...
            $('ul.chat-user-list').html(htmllist[2]);
            if (activeid != '') {
                $('ul.chat-user-list li[data-userid=' + activeid + ']').addClass('active');
            }
            $(".chat-user-list li").each(function (index, item) {
...
$(document).ready(function () {
    $("#search_msg").on("keyup", function () {
        ...
        $("ul.chat-user-list li").filter(function () {
