            ...
            jQuery.ajax({
                ...
                success: function (result) {
                    ...
                    if(result.recordsTotal == 0 || result.found == 0 && ignoreItems == '' ) {
                        ...
                        $('#frmSectionEventsModule').find('#addSection').attr('disabled','disabled');
                    }else{
                        $('#frmSectionEventsModule').find('#addSection').removeAttr('disabled');
                    }
                }
                },
...
$('#sectionEventsModule').on('shown.bs.modal', function () {
    ...
    EventsDataTable.init(start, range,false);
                ...
                if ($('input[name=found]').val() > 0) {
                    ...
                    setTimeout(function () {
                        if ($('#sectionEventsModule #record-table tr').length < $('input[name=total_records]').val()) {
                            ...
                            EventsDataTable.init(start, range,true);
                        } else {
                            $('#datatable_events_ajax').closest('.col-md-12').loading('stop');
                        }
                    }, 1000);
