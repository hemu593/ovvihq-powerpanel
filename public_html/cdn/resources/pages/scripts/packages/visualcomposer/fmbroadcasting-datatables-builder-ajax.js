            ...
            jQuery.ajax({
                ...
                success: function (result) {
                    ...
                    if(result.recordsTotal == 0 || result.found == 0 && ignoreItems == '' ) {
                        ...
                        $('#frmSectionFMBroadcastingModule').find('#addSection').attr('disabled','disabled');
                    }else{
                        $('#frmSectionFMBroadcastingModule').find('#addSection').removeAttr('disabled');
                    }
                      }
                },
...
$('#sectionFMBroadcastingModule').on('shown.bs.modal', function () {
    ...
    FMBroadcastingDataTable.init(start, range,false);
                ...
                if ($('input[name=found]').val() > 0) {
                    ...
                    setTimeout(function () {
                        if ($('#sectionFMBroadcastingModule #record-table tr').length < $('input[name=total_records]').val()) {
                            ...
                            FMBroadcastingDataTable.init(start, range,true);
                        } else {
                            $('#datatable_fmbroadcasting_ajax').closest('.col-md-12').loading('stop');
                        }
                    }, 1000);
