...
var TeamModule = function() {
    ...
    return {
        ...
        moduleSectionsTeam: function(caption, config, configTxt, recids, recTitle, edit, template, layoutType, extra_class) {
            let manageRecordURL = baseURL + 'team';
            recids = recids.split(',');
            ...
            if (edit == 'N') {
                ...
                section += '</a>';
                section += '</div>';
                section += '<input module="team" id="' + iCount + '" data-extclass="' + extra_class + '" data-filter="' + template + '" type="hidden" data-config="' + config + '" data-layout="' + layoutType + '" data-caption="' + caption + '" data-type="module" value="team" />';
                section += '<div class="clearfix"></div>';
                section += '</div>';
                ...
                section += '</a>';
                section += '</div>';
                section += '<input module="team" id="' + edit + '" data-extclass="' + extra_class + '" data-filter="' + template + '" type="hidden" data-config="' + config + '" data-layout="' + layoutType + '" data-caption="' + caption + '" data-type="module" value="team" />';
                section += '<div class="clearfix"></div>';
                $('#section-container').find('div[data-editor=' + edit + ']').html(section);

            }
        },
        teamTemplate: function(val, template, edit,extra_class) {
            let manageRecordURL = baseURL + 'team';
            var section = '';
            if (edit == 'N') {
                var iCount = 'item-' + ($('.ui-state-default').length + 1);
                ...
                section += '<div class="col-md-12"><label><b>' + val + '</b></label> <a class="" target="_blank" title="Manage Records" href="' + manageRecordURL + '">Manage Records</a> <ul><li>Template: ' + template + '</li></ul></div>';
                section += '<input data-extclass="' + extra_class + '" id="' + iCount + '"  data-type="' + template + '"  type="hidden" module="team" class="txtip" value="' + val + '"/>';
                section += '<div class="clearfix"></div>';
                section += '</div>';
                section += '</div>';
                ...
                section += '<div class="col-md-12"><label><b>' + val + '</b></label> <a class="" target="_blank" title="Manage Records" href="' + manageRecordURL + '">Manage Records</a> <ul><li>Template: ' + template + '</li></ul></div>';
                section += '<input id="' + edit + '" data-extclass="' + extra_class + '"  data-type="' + template + '"  module="team" type="hidden" class="txtip" value="' + val + '"/>';
                section += '<div class="clearfix"></div>';
                $('#section-container').find('div[data-editor=' + edit + ']').html(section);
            }
        },
        submitFrmSectionTeamModuleTemplate: function() {
            if ($('#frmSectionTeamModuleTemplate').validate().form()) {
                ...
                TeamModule.teamTemplate(val,  template, edit,extra_class);
    ...
    return {
        ...
        init: function(from, to,pagination) {
            ...
            jQuery.ajax({
                ...
                complete: function () {
                    if ($('#sectionTeamModule #record-table tr').length > 1) {
                        $('#sectionTeamModule #record-table #not-found').remove();
                    }
                    $('#datatable_team_ajax').closest('.col-md-12').loading('stop');
                }
            });
        }
    };
...
$('#sectionTeamModule').on('shown.bs.modal', function() {
    ...
    teamDataTable.init(start, range,false);
            ...
            onTotalScroll: function() {
                if ($('input[name=found]').val() > 0) {
                    if ($('#sectionTeamModule').find('#record-table tr').length < $('input[name=total_records]').val()) {
                        ...
                        teamDataTable.init(start, range,true);
                    }
                }
            }
        }
