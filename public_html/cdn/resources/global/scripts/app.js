...
function search() {
    ...
    if ((searchData.length) > 2) {
        $.ajax({
            ...
            autoFocus: true,
            dataType: 'json',
            url: site_url + '/powerpanel/search/auto-complete',
            data: ({
                searchValue: searchData
            }),
            beforeSend: function(xhr) {
            	$("#search-dropdown .resultdata").html('<div class="genloader text-center"><i class="ri-loader-2-line p-3"></i></div>');
                xhr.setRequestHeader('X-CSRF-Token', $('input[name="_token"]').val());
            },
            success: function(data) {
              $("#search-dropdown .resultdata").html(data.resulthtml);
              if(data.resultcount == 0){
              	$("#search-dropdown #view_all_result").hide();
              }else{
              	$("#search-dropdown #view_all_result").show();
              }
            },
            complete: function() {
            },
...
$('#view_all_result').click(function() {
   var term = $('#search-options').val();
   window.location.href = site_url+"/powerpanel/search?searchValue="+encodeURIComponent(term); 
});
...
$('select[name=locale]').change(function() {
 ...
 if (!super_admin) {
    ...
    if (termsAccepted == 'N') {
        const modal = new bootstrap.Modal('#tnc',{
            backdrop: 'static',
            keyboard: false
        });
        modal.show();
...
function GetLockData(id, uid, mid, url) {
    ...
    if (confirm("Are you sure you want to lock page? Click on OK to confirm.")) {
        $.ajax({
            url: ajaxurl,
            data: {
                id: id,
...
function GetUnLockData(id, uid, mid, url) {
    ...
    if (confirm("Are you sure you want to unlock page? Click on OK to confirm.")) {
        $.ajax({
            url: ajaxurl,
            data: {
                id: id,
