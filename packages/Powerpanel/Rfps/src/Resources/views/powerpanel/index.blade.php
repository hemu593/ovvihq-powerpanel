                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-shrink-0">
                        @can('rfps-create')
                            ...
                            <a class="btn btn-light bg-gradient waves-effect waves-light btn-label" href="{{ url('powerpanel/rfps/add') }}">
                                <div class="d-flex">
                                    <div class="flex-shrink-0">
                                        <i class="ri-add-line label-icon align-middle fs-20 me-2"></i>
                                    </div>
                                    <div class="flex-grow-1">
                                        {{ trans('rfps::template.rfpsModule.addRfps') }}
                                    </div>
                                </div>
                            </a>
                            @endif
                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-2 col-sm-4" id="hidefilter">
                        <select class="form-control" id="statusfilter" data-choices data-choices-search-false>
                            <option value="">{!! trans('rfps::template.common.selectstatus') !!}</option>
                            <option value="Y">{!! trans('rfps::template.common.publish') !!}</option>
                            <option value="N">{!! trans('rfps::template.common.unpublish') !!}</option>
                        </select>
                    </div>

                    @can('rfps-category-list')
                        @if(isset($categories))
                            <div class="col-lg-2 col-sm-4">
                            {!! $categories !!}
                            </div>
                    ...
                    <div class="col-lg-4 col-sm-6">
                        <div class="input-group input-daterange">
                            ...
                            <input type="text" class="form-control" id="start_date" name="start_date" placeholder="{{ trans('rfps::template.common.startdate') }}" readonly data-provider="flatpickr" data-date-format="{{Config::get('Constant.DEFAULT_DATE_FORMAT')}}">
                            <input type="text" class="form-control" id="end_date" name="end_date" placeholder="{{ trans('rfps::template.common.enddate') }}" readonly data-provider="flatpickr" data-date-format="{{Config::get('Constant.DEFAULT_DATE_FORMAT')}}">
                            <button class="btn btn-outline-primary border-outline-light btn-rh-search" id="rfpsRange" type="button"><i class="ri-search-line fs-13"></i></button>
                        </div>
                    </div>
            ...
            @elseif($iTotalRecords > 0)
                <div class="card-body">
                    <div class="live-preview">
                        ...
                        @endphp
                        @include('powerpanel.partials.datatable-view',['ModuleName'=>'Rfps','Permission_Delete'=>'rfps-delete','tablearray'=>$tablearray,'userIsAdmin'=>$userIsAdmin,'Module_ID'=>Config::get('Constant.MODULE.ID')])
                    </div>
                </div><!-- end card-body -->
                <!-- Modal -->
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/quickeditpopup.blade.php') != null)
                @include('powerpanel.partials.quickeditpopup',['TableName'=>'rfps'])
                @endif
                @else
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/addrecordsection.blade.php') != null)
                @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/rfps/add')])
                @endif
            @endif
        </div><!-- end card -->
    </div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/rfps/DeleteRecord") !!}';
    var rollbackRoute = window.site_url + "/powerpanel/rfps/rollback-record";
    var APPROVE_URL = '{!! url("/powerpanel/rfps/ApprovedData_Listing") !!}';
    var getChildData = window.site_url + "/powerpanel/rfps/getChildData";
    var getChildData_rollback = window.site_url + "/powerpanel/rfps/getChildData_rollback";
    var ApprovedData_Listing = window.site_url + "/powerpanel/rfps/ApprovedData_Listing";
    var Get_Comments = window.site_url + "/powerpanel/rfps/Get_Comments";
...
<script src="{{ $CDN_PATH.'resources/pages/scripts/packages/rfps/rfps-datatables-ajax.js' }}" type="text/javascript"></script>
