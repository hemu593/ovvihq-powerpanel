                    ...
                    <div class="right_content">
                        <div class="back_div">
                            ...
                            if($Nurl == "?N=N"){
                                ...
                                $backpageUrl = App\Helpers\MyLibrary::getFront_Uri('decisions-category')['uri'].$Nurl.$catid.$start_date_time.$end_date_time.$page;
                            }
                        ...
                        <h1 class="cms_detail_h2 ac-mb-xs-15">{{ $decisions->varTitle }}</h1>
                        <h6 class="ac-mb-xs-15">Published: {{ date('l F jS, Y',strtotime($decisions->dtDateTime)) }}</h6>
                         @if(!empty($decisions->txtDescription))
                        {!! htmlspecialchars_decode($txtDescription) !!}
                        @endif
                        @if(!empty($decisions->fkIntDocId))
                        @php
                        $docsAray = explode(',', $decisions->fkIntDocId);
