                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-6 col-md-3 col-xs-3">
						<div class="input-group new_date_picker date-picker input-daterange">
                            ...
                            <input type="text" class="form-control" id="start_date" name="start_date" placeholder="From Date" readonly data-provider="flatpickr" data-date-format="{{Config::get('Constant.DEFAULT_DATE_FORMAT')}}">
                            <span class="input-group-text"><i class="ri-calendar-line"></i></span>
                            ...
                            <button class="btn btn-outline-primary btn-rh-search" id="complaintleadrange" type="button"><i class="ri-search-line"></i></button>
                        </div>
                    </div>
                    <!--end col-->
                            ...
                            <table class="table table-striped table-bordered table-hover table-checkable hide-mobile" id="datatable_ajax">
                                <thead class="text-muted table-light">
                                    <tr role="row" class="heading">
                                        ...
                                        <th width="10%" align="left">{{ trans('polls::template.common.title') }}</th>
                                        <th width="8%" align="center">{{ trans('Form Details') }}</th>
                                        <th width="10%" align="center">{{ trans('polls::template.common.message') }}</th>
                                        <th width="10%" align="center">{{ trans('polls::template.common.received_date_time') }}</th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
                            @can('online-polling-lead-delete')
                            <a href="javascript:;" class="btn-sm rounded-pill btn btn-danger right_bottom_btn deleteMass">{{ trans('polls::template.common.delete') }}</a>
                            @endcan
                            <a href="#selectedRecords" class="btn-sm rounded-pill btn btn-primary right_bottom_btn ExportRecord" data-bs-toggle="modal">{{ trans('polls::template.onlinepollingleadModule.export') }}</a>
                        </div>
                    </div>
                </div><!-- end card-body -->
            @else
        ...
        <div class="modal-vertical">	
            <div class="modal-content">
                <div class="modal-header">
                    {{ trans('polls::template.common.alert') }}
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"> </button>
                </div>
                <div class="modal-body text-center">{{ trans('polls::template.onlinepollingleadModule.noExport') }} </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">{{ trans('polls::template.common.ok') }}</button>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="modal-body text-center">
					<div class="row">
						<div class="col-12 mb-3">
                            {{ trans('polls::template.onlinepollingleadModule.recordsExport') }}
						</div>
						<div class="col-12">
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" value="selected_records" id="selected_records" name="export_type">
								<label for="form-check-label">{{ trans('polls::template.onlinepollingleadModule.selectedRecords') }}</label>
							</div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" value="all_records" id="all_records" name="export_type" checked>
								<label for="form-check-label">{{ trans('polls::template.onlinepollingleadModule.allRecords') }}</label>
							</div>
						</div>
					</div>
				</div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" id="ExportRecord" data-bs-dismiss="modal">{{ trans('polls::template.common.ok') }} </button>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-vertical">	
            <div class="modal-content">
                ...
                <div class="modal-body text-center">{{ trans('polls::template.onlinepollingleadModule.leastRecord') }} </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">{{ trans('polls::template.common.ok') }} </button>
                </div>
            </div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/online-polling-lead/DeleteRecord") !!}';
</script>
...
<script src="{{ $CDN_PATH.'resources/pages/scripts/packages/poll/polllead-datatables-ajax.js' }}" type="text/javascript"></script>
