...
/**
 * The FormsAndFees class handels bannner queries
 * ORM implemetation.
...
namespace Powerpanel\FormsAndFees\Models;
...
class FormsAndFees extends Model {
     ...
     * @var array
     */
    protected $table = 'forms_and_fees';
    protected $fillable = [
        'id',
        'fkMainRecord',
        'varTitle',
    ...
    public static function getRecords() {
        $response = false;
        $response = Cache::tags(['FormsAndFees'])->get('getCareersRecords');
        if (empty($response)) {
            $response = Self::Select(['id', 'varTitle',    'varSector',  'intSearchRank', 'txtDescription', 'chrMain', 'chrAddStar', 'intDisplayOrder', 'chrPublish', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'])
                    ->deleted()
                    ->publish()
                    ->paginate(10);
            Cache::tags(['FormsAndFees'])->forever('getCareersRecords', $response);
        }
        return $response;
    }
    ...
    public static function getFrontDetail($aliasID) {
        $response = false;
        $response = Cache::tags(['FormsAndFees'])->get('getCareersRecordIdByAliasID_' . $aliasID);
        if (empty($response)) {
            $moduleFields = [
                'id',
                'fkMainRecord',
            ...
            $response = Self::Select($moduleFields)
                    ...
                    ->first();
            Cache::tags(['FormsAndFees'])->forever('getCareersRecordIdByAliasID_' . $aliasID, $response);
        }
        return $response;
    }

    
    public static function getAllFormsandfees($limit,$dbFilter=false,$sector_slug) {
        $response = false;
        $moduleFields =  [
                'id',
                'fkMainRecord',
...
//            echo '<pre>';print_r($sector_slug);exit;
            if(isset($sector_slug) && $sector_slug != 'ofreg' ) {
                $response->where('varSector', '=', $sector_slug);
            }
...
//                     ->where('varSector', '=', $sector_slug)
    ...
    public function child() {
		return $this->hasMany('Powerpanel\FormsAndFees\Models\FormsAndFees', 'fkMainRecord', 'id');
	}
    ...
    public static function getChildGrid() {
        ...
        $id = $_REQUEST['id'];
        $response = false;
        $moduleFields = ['id', 'varTitle','fkMainRecord' ,'varSector',  'intSearchRank','intApprovedBy','txtDescription', 'chrMain', 'chrAddStar', 'intDisplayOrder', 'chrApproved','UserID','chrPublish', 'chrPageActive', 'varPassword', 'chrDraft', 'dtApprovedDateTime','chrTrash', 'FavoriteID', 'created_at', 'updated_at'];
    ...
    public static function getChildrollbackGrid() {
        $id = $_REQUEST['id'];
        $response = false;
        $moduleFields = ['id', 'varTitle',    'varSector',  'intSearchRank','txtDescription', 'chrMain', 'chrAddStar', 'intDisplayOrder', 'chrPublish', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'];
        $response = Self::getPowerPanelRecords($moduleFields, false)
                ->deleted()
                ->where('chrMain', 'N')
                ->where('chrRollBack', 'Y')
    ...
    public static function approved_data_Listing($request) {
        ...
        CommonModel::updateRecords($whereConditions, $updateMainRecord, false, 'Powerpanel\FormsAndFees\Models\FormsAndFees');
...
public static function getBuilderRecordList($filterArr = [])
    {
        ...
        $moduleFields = [
            'forms_and_fees.id',
            'forms_and_fees.intAliasId',
            'forms_and_fees.varTitle',
            'forms_and_fees.varSector',
           
            DB::raw('IFNULL(COUNT(nq_page_hits.id), 0) AS hits'),
            'forms_and_fees.chrPublish',
            'forms_and_fees.chrDelete',
            'forms_and_fees.updated_at',
        ];
        $response = Self::getPowerPanelRecords($moduleFields, false, false, false, false)
            ->filter($filterArr);
        $response = $response->leftJoin('page_hits', 'forms_and_fees.intAliasId', '=', 'page_hits.fkIntAliasId')
            ->where('forms_and_fees.chrPublish', 'Y')
            ->where('forms_and_fees.chrDelete', 'N')
            ->where('forms_and_fees.chrMain', 'Y')
            ->where('forms_and_fees.chrTrash', '!=', 'Y')
            ->where('forms_and_fees.chrDraft', '!=', 'D')
            ->where('forms_and_fees.chrIsPreview', 'N')
            ->groupBy('forms_and_fees.id')
            ->get();
        return $response;
    }
}
