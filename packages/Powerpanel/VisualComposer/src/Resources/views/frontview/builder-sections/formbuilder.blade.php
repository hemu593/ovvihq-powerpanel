...
@endif
    ...
    @if(Request::segment(1) == 'foi' && Request::segment(2) == '')
        {{-- ------------------------------------------------------------
            For FOI page form design
        ------------------------------------------------------------ --}}
        <div class="contact-wrap">
            <div class="desc n-mb-lg-40">
                <p><span class="s-title">CAN'T FIND WHAT YOU’RE LOOKING FOR?</span>We are here to help. Please submit your request by completing the form below.</p>
            </div>
            {!! Form::open(['method' => 'POST','url'=> url('/formbuildersubmit'),'class'=>'nqform','id'=>'formbuildername','enctype'=>'multipart/form-data']) !!}
                    ...
                    @foreach($data['formdata'] as $formdata)
                        ...
                        @if($formdata['type'] == 'file')
                            <div class="form-group ac-form-group">
                                <div class="upload-sec">
                                    <i class="n-icon" data-icon="s-attach"></i>
                                    <label class="ac-file-label" for="customFile">{{ $formdata['label'] }}</label>
                                    <input type="{{ $formdata['type'] }}" name='{{ $formdata['name'] }}' id='{{ $formdata['name'] }}' class="ac-file-input" placeholder="Attach file">
                            ...
                            <div class="ac-form-group">
                                ...
                                @if(isset($formdata['className']))
                                    ...
                                    @else
                                        ...
                                        @elseif($formdata['subtype'] == 'email')
                                            <input type="{{ $formdata['subtype'] }}" value="{{ (!empty($formdata['value']) ? $formdata['value'] : '') }}" autocomplete="off"  maxlength="{{ $maxlength }}" placeholder="{{ $placeholder }}" name='{{ $formdata['name'] }}' id='{{ $formdata['name'] }}' class="ac-input form-control {{ $className }}"> 
                                        @else
                                            <input type="{{ $formdata['subtype'] }}" value="{{ (!empty($formdata['value']) ? $formdata['value'] : '') }}" autocomplete="off" maxlength="{{ $maxlength }}" placeholder="{{ $placeholder }}" name='{{ $formdata['name'] }}' id='{{ $formdata['name'] }}' class="ac-input form-control {{ $className }}"> 
                                        @endif
                                    @endif
                        ...
                        @if($formdata['type'] == 'textarea')
                            <div class="ac-form-group">
                                ...
                                <textarea type="{{ $formdata['subtype'] }}" class="ac-textarea form-control {{ $className }}" name='{{ $formdata['name'] }}' id='{{ $formdata['name'] }}' maxlength="{{ $maxlength }}" placeholder="{{ $placeholder }}">{{ (!empty($formdata['value']) ? $formdata['value'] : '') }}</textarea>
                ...
                <!-- Captcha -->
                <div class="captcha_builder clearfix"></div>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group ac-form-group n-mb-md-0">
                            <div id="foi_html_element" class="g-recaptcha"></div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group ac-form-group n-mb-md-0 n-tar-sm n-tal">
                            <button type="submit" class="ac-btn ac-btn-primaryr" title="Submit">Send</button>
                        </div>
                    </div>
                </div>
            {!! Form::close() !!}
    ...
    @else
        {{-- ------------------------------------------------------------
             Normal Form Design For all Page (Main Code)
        ------------------------------------------------------------ --}}
...
<script>
    ...
    var hiddenRecaptcha1 = 'g-recaptcha-response';
    var onFoiloadCallback = function () {
        grecaptcha.render('foi_html_element', {
            'sitekey': sitekey
        });
    };
...
"g-recaptcha-response": {
            ...
            errorPlacement: function (error, element) {
                ...
                else if (element.attr('id') == 'g-recaptcha-response-1') {
...
<script src="https://www.google.com/recaptcha/api.js?onload=onFoiloadCallback&render=explicit" async defer></script>
