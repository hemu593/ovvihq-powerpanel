<?php
namespace Powerpanel\PublicationsCategory\Models;
use DB;
use Illuminate\Database\Eloquent\Model;
use Cache;
use App\CommonModel;
class PublicationsCategory extends Model {
    protected $table = 'publications_category';
    protected $fillable = [
        'id',
        'varTitle','varSector',
        'intAliasId',
    ...
    public static function getRecordIdByAliasID($aliasID) {
        $response = false;
        $response = Cache::tags(['PublicationsCategory'])->get('getPublicationsCatRecordIdByAliasID' . $aliasID);
        if (empty($response)) {
            $response = Self::Select('id')
                    ...
                    ->first();
            Cache::tags(['PublicationsCategory'])->forever('getPublicationsCatRecordIdByAliasID' . $aliasID, $response);
        }
        return $response;
    }
    ...
    public static function getFrontList() {
        $response = false;
        $publicationsCategoryFields = ['id', 'varTitle'];
        $response = Cache::tags(['PublicationsCategory'])->get('getFrontServiceCatList');
        if (empty($response)) {
            $response = Self::getFrontRecords($publicationsCategoryFields)
                    ->deleted()
                    ->publish()
                    ->get()
                    ->pluck('varTitle', 'id','varSector');
            Cache::tags(['PublicationsCategory'])->forever('getFrontServiceCatList', $response);
        }
        return $response;
    }
    /**
    ...
    public static function getAllCategoriesFrontSidebarList() {
        $response = false;
        $publicationsCategoryFields = ['id', 'intAliasId', 'varTitle','varSector', 'intParentCategoryId'];
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['PublicationsCategory'])->get('getFrontSidebarCatList');
        if (empty($response)) {
            $response = Self::getFrontRecords($publicationsCategoryFields, $aliasFields)
                    ->checkMainRecord('Y')
                   ->where('chrIsPreview', 'N')
                ->where('chrTrash', '!=', 'Y')
                ->where('chrDraft', '!=', 'D')
                    ...
                    ->publish()
                    ->get();
            Cache::tags(['PublicationsCategory'])->forever('getFrontSidebarCatList', $response);
        }
        return $response;
    }
    /**
     ...
     * @author  NetQuick
     */
    public function publications_category() {
        $response = false;
        $response = $this->hasOne('Powerpanel\PublicationsCategory\Models\PublicationsCategory', 'id', 'intParentCategoryId');
        return $response;
    }
    /**
    ...
    public function parentCategory() {
        $response = false;
        $response = $this->belongsTo('App\PublicationsCategory', 'intParentCategoryId', 'id');
        return $response;
    }
    /**
    ...
    public function child() {
		return $this->hasMany('Powerpanel\PublicationsCategory\Models\PublicationsCategory', 'fkMainRecord', 'id');
	}
    ...
    public static function getRecordListforPublicationsCategoryGrid($filterArr = false, $isAdmin = false, $ignoreId = array(), $userRoleSector) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleFields = [
            'id',
        ...
        $query = $query->deleted()
                ...
                if(!$isAdmin){
                    $query->where('varSector', $userRoleSector);
                }
        $response = $query->get();
        return $response;
    ...
    public static function getFavoriteRecordListforPublicationsCategoryGrid($filterArr = false, $isAdmin = false, $ignoreId = array(), $userRoleSector) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleFields = [
            'id',
    ...
    public static function getDraftRecordListforPublicationsCategoryGrid($filterArr = false, $isAdmin = false, $ignoreId = array(), $userRoleSector) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleFields = [
            'id',
    ...
    public static function getTrashRecordListforPublicationsCategoryGrid($filterArr = false, $isAdmin = false, $ignoreId = array(), $userRoleSector) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleFields = [
            'id',
    ...
    public static function getRecordListforGridbyIds($ids, $filterArr = false) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleField = ['publications_category.id',
                        'publications_category.fkMainRecord',
                        'publications_category.intAliasId',
                        'publications_category.varTitle',
                        'publications_category.varSector',
                        'publications_category.intParentCategoryId',
                        'publications_category.intDisplayOrder',
                        'publications_category.txtDescription',
                        'publications_category.chrPublish',
                        'publications_category.LockUserID',
                        'publications_category.chrLock',
                        'publications_category.dtDateTime',
                        'publications_category.dtEndDateTime',
                        'publications_category.chrPageActive',
                        'publications_category.varPassword',
                        'publications_category.chrAddStar',
                        'publications_category.chrDraft',
                        'publications_category.intSearchRank',
                        'publications_category.chrTrash',
                        'publications_category.FavoriteID',
                        'publications_category.created_at',
                        'publications_category.updated_at'];
        $query = Self::select($moduleField)
                        ->leftjoin('publications_category as Og1', 'publications_category.intParentCategoryId', '=', 'Og1.id')
                        ->whereIn('publications_category.id', $ids)
                        ->where(function ($query) use ($userid) {
                            $query->where("publications_category.UserID", '=', $userid)
                            ->where('publications_category.chrPageActive', '=', 'PR')
                                    ->orWhere('publications_category.chrPageActive', '!=', 'PR');
                            })
                        ->where('publications_category.chrMain', 'Y')
                        ->where('publications_category.chrIsPreview', 'N')
                        ->where('publications_category.chrTrash', '!=', 'Y');
        if (!empty($filterArr['orderByFieldName']) && !empty($filterArr['orderTypeAscOrDesc'])) {
            $query->orderBy('publications_category.' . $filterArr['orderByFieldName'], $filterArr['orderTypeAscOrDesc']);
        }
        $response = $query->groupBy('publications_category.id')->get();
        return $response;
    }
    /**
     * This method handels retrival of backend record list
    ...
    public static function getRecordListforSelectBoxbyIds($ids,$admin = false) {
        ...
        $response = false;
        $response = DB::table('publications_category AS Og')
                ->leftjoin('publications_category as Og1', 'Og.intParentCategoryId', '=', 'Og1.id')
    ...
    public static function getParentNodesIdsByRecordId($recordId = false) {
        $response = array();
        if ($recordId) {
            $tree = DB::select("select GetAncestry_PublicationsCategory(?) AS tree", [$recordId]);
    ...
    public static function getRecordsForChart($filterArr = false) {
        ...
        $response = self::select($moduleFields);
        if ($parentCategoryFields != false) {
            $data['childpublications_category'] = function ($query) use ($parentCategoryFields) {
                $query->select($parentCategoryFields);
            };
        }
        if (count($data) > 0) {
    ...
    public static function getCategoryList($paginate = 6, $page) {
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['PublicationsCategory'])->get('getCategoryList_' . $page);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ...
                    ->checkMainRecord('Y')
                    ->paginate($paginate);
            Cache::tags(['PublicationsCategory'])->forever('getCategoryList_' . $page, $response);
        }
        return $response;
    }
    /**
    ...
    public static function getFrontDetail($id) {
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['PublicationsCategory'])->get('getPublicationsCategoryFrontDetail_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->where('chrMain', 'Y')
                    ->first();
            Cache::tags(['PublicationsCategory'])->forever('getPublicationsCategoryFrontDetail_' . $id, $response);
        }
        return $response;
    }
    /**
    ...
    public static function UpdateDisplayOrder($order, $parentRecordId = false) {
        $updateSql = "UPDATE nq_publications_category SET `intDisplayOrder` = `intDisplayOrder` + 1
    ...
    public static function updateherarachyRecords($when, $ids) {
        $response = false;
        $response = DB::statement("UPDATE nq_publications_category SET intDisplayOrder = (CASE id " . $when . " ELSE intDisplayOrder END) WHERE id BETWEEN " . min($ids) . " AND " . max($ids) . " and chrDelete = 'N' and chrMain='Y' and chrIsPreview='N'");
        return $response;
    }
    public static function updateDisplayOrderByParent($id, $newOrder, $parentRecordId) {
        $response = false;
        $updateQuery = "UPDATE nq_publications_category SET intDisplayOrder=intDisplayOrder + 1 WHERE intDisplayOrder >= " . $newOrder . " and chrDelete = 'N' AND id!=$id AND chrDelete = 'N' AND intParentCategoryId=$parentRecordId";
    ...
    public static function approved_data_Listing($request) {
        ...
        CommonModel::updateRecords($whereConditions, $updateMainRecord,false,'Powerpanel\PublicationsCategory\Models\PublicationsCategory');
        ...
        CommonModel::updateRecords($whereConditions_ApproveN, $updateToApproveN,false,'Powerpanel\PublicationsCategory\Models\PublicationsCategory');
        ...
        CommonModel::updateRecords($whereConditionsApprove, $updateToApprove,false,'Powerpanel\PublicationsCategory\Models\PublicationsCategory');
...
//        echo '<pre>';print_r($response->get());exit;
        return $response;
    }
    /**
     * This method handels filter scope
    ...
    public static function getPublicationsCategoryNameByPublicationsCategoryId($ids) {
        $response = false;
...
public function childpublications_category() {
