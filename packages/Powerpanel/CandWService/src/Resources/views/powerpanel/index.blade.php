            ...
            <div class="card-header border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-shrink-0">
                        @can('candwservice-create')
                            @if( isset(App\Helpers\MyLibrary::getFront_Uri('candwservice')['uri']) )
                            <a class="btn btn-light bg-gradient waves-effect waves-light btn-label" href="{{ url('powerpanel/candwservice/add') }}">
                                <div class="d-flex">
                                    <div class="flex-shrink-0">
                                        <i class="ri-add-line label-icon align-middle fs-20 me-2"></i>
                                    </div>
                                    <div class="flex-grow-1">
                                        {{ trans('candwservice::template.candwserviceModule.addCandWService') }}
                                    </div>
                                </div>
                            </a>
                            @endif
                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-2 col-sm-4" id="hidefilter">
                        <select class="form-control" id="statusfilter" data-choices data-choices-search-false>
                            <option value="">{!! trans('candwservice::template.common.selectstatus') !!}</option>
                            <option value="Y">{!! trans('candwservice::template.common.publish') !!}</option>
                            <option value="N">{!! trans('candwservice::template.common.unpublish') !!}</option>
                        </select>
                    </div>
            ...
            @if($iTotalRecords > 0)
                <div class="card-body">
                    <div class="live-preview">
                        ...
                        @endphp
                        @include('powerpanel.partials.datatable-view',['ModuleName'=>'C&W Service','Permission_Delete'=>'candwservice-delete','tablearray'=>$tablearray,'userIsAdmin'=>$userIsAdmin,'Module_ID'=>Config::get('Constant.MODULE.ID')])
                    </div>
                </div><!-- end card-body -->
                <!-- Modal -->
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/quickeditpopup.blade.php') != null)
                @include('powerpanel.partials.quickeditpopup',['TableName'=>'candwservice'])
                @endif
                @else
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/addrecordsection.blade.php') != null)
                @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/candwservice/add')])
                @endif
            @endif
        </div><!-- end card -->
    </div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/candwservice/DeleteRecord") !!}';
    var rollbackRoute = window.site_url + "/powerpanel/candwservice/rollback-record";
    var APPROVE_URL = '{!! url("/powerpanel/candwservice/ApprovedData_Listing") !!}';
    var getChildData = window.site_url + "/powerpanel/candwservice/getChildData";
    var getChildData_rollback = window.site_url + "/powerpanel/candwservice/getChildData_rollback";
    var ApprovedData_Listing = window.site_url + "/powerpanel/candwservice/ApprovedData_Listing";
    var Get_Comments = window.site_url + "/powerpanel/candwservice/Get_Comments";
...
<script src="{{ $CDN_PATH.'resources/pages/scripts/packages/candwservice/candwservice-datatables-ajax.js' }}" type="text/javascript"></script>
