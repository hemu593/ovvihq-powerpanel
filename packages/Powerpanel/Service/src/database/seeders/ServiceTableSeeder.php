...
class ServiceTableSeeder extends Seeder
{
     ...
     public function run()
    {
        ...
        if (!isset($exists->id)) {
            ...
            //Adding Service Module In visual composer
        if (Schema::hasTable('visualcomposer')) {
            $netServiceModule = DB::table('visualcomposer')->select('id')->where('varTitle', 'Service')->where('fkParentID', '0')->first();

            if (!isset($netServiceModule->id) || empty($netServiceModule->id)) {
            ...
            $netServiceChild = DB::table('visualcomposer')->select('id')->where('varTitle', 'Service')->where('fkParentID', '<>', '0')->first();

            if (!isset($netServiceChild->id) || empty($netServiceChild->id)) {
                DB::table('visualcomposer')->insert([
                    'fkParentID' => $netServiceModule->id,
                    'varTitle' => 'Service',
                    'varIcon' => 'fa fa-cogs',
                    'varClass' => 'service',
                    'varTemplateName' => 'service::partial.service',
                    'varModuleID' => $pageModuleCode->id,
                    'created_at' => Carbon::now(),
                    'updated_at' => Carbon::now(),
                ]);
            ...
            $latestBlog = DB::table('visualcomposer')->select('id')->where('varTitle', 'All Service')->where('fkParentID', '0')->first();

            if (!isset($latestBlog->id) || empty($latestBlog->id)) {
                DB::table('visualcomposer')->insert([
                    'fkParentID' => $netServiceModule->id,
                    'varTitle' => 'All Service',
                    'varIcon' => 'fa fa-cogs',
                    'varClass' => 'service-template',
                    'varTemplateName' => 'service::partial.all-service',
                    'varModuleID' => $pageModuleCode->id,
                    'created_at' => Carbon::now(),
                    'updated_at' => Carbon::now(),
                ]);
