    ...
    <div class="col-xl-12">
        ...
        <div class="card">
        	@if($iTotalRecords > 0)
                <div class="card-header cmpage-topheader border border-dashed border-end-0 border-start-0 border-top-0">
                    <div class="d-xl-flex flex-wrap align-items-center">
                        ...
                        <div class="flex-shrink-0 addpage-btn order-sm-2">
                            @can('service-create')
                                <a class="btn btn-light btn-theme bg-gradient waves-effect waves-light btn-label" href="{{ url('powerpanel/service/add') }}">
                                    <div class="d-flex">
                                        <div class="flex-shrink-0">
                                            <i class="ri-add-line label-icon align-middle fs-20 me-2"></i>
                                        </div>
                                        <div class="flex-grow-1">
                                            {{ trans('service::template.serviceModule.addService') }}
                                        </div>
                                    </div>
                                </a>
                            @endcan
                        ...
                        <div class="cm-filter flex-grow-1 order-sm-1 d-flex align-items-center">
                                    ...
                                    <div class="p-3 dropdown-body">
                                        <div class="mb-3" id="hidefilter">
                                            <select class="form-select" id="statusfilter">
                                                <option value="">{!! trans('service::template.common.selectstatus') !!}</option>
                                                <option value="Y">{!! trans('service::template.common.publish') !!}</option>
                                                <option value="N">{!! trans('service::template.common.unpublish') !!}</option>
                                            </select>
                                        </div>
                                        @if($userIsAdmin)
                                            <div class="mb-3">
                                            ...
                                            <select class="form-select" id="category" name="category">
                                                <option value="">Select Category</option>
                                                    @if(!empty($categories))
                                                        ...
                                                        @php $selected = ''; @endphp
                                                        @if(isset($services->intFKCategory))
                                                            @if($cat['id'] == $services->intFKCategory)
                                                                @php $selected = 'selected'; @endphp
                                                            @endif
                                                        @endif
                                                        <option value="{{ $cat['id'] }}" {{ $selected }}>{{$cat['varTitle'] }}</option>
                                                        @endforeach
                                                    @endif
                                            </select>
                                        </div>
                            ...
                            <div class="filter-search d-inline-block">
                                <input type="search" class="form-control search" placeholder="Search by Service" id="searchfilter">
                    ...
                    <div class="live-preview">
                        <div class="table-responsive">
                            ...
                            $tablearray = [
                                'DataTableTab'=>[
                                    'ColumnSetting'=>[
                                        ['Identity_Name'=>'title','TabIndex'=>'2','Name'=>'Service Name'],
                                        ['Identity_Name'=>'cat','TabIndex'=>'3','Name'=>'Category'],
                                        ['Identity_Name'=>'sdate','TabIndex'=>'4','Name'=>'Service Code'],
                                    ...
                                    'DataTableHead'=>[
                                        ['Title'=>'Service Name','Align'=>'left'],
                                        ['Title'=>'Category','Align'=>'center'],
                                        ['Title'=>'Service Code','Align'=>'center'],
                            ...
                            @endphp
                            @include('powerpanel.partials.datatable-view',['ModuleName'=>'Service','Permission_Delete'=>'service-delete','tablearray'=>$tablearray,'userIsAdmin'=>$userIsAdmin,'Module_ID'=>Config::get('Constant.MODULE.ID')])
                        </div>
                    </div>
                </div><!-- end card-body -->
                ...
                @include('powerpanel.partials.quickeditpopup',['TableName'=>'service'])
                @endif
                @else
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/addrecordsection.blade.php') != null)
                @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/service/add')])
                @endif
            @endif
...
@section('scripts')
    <script type="text/javascript">
        window.site_url = '{!! url("/") !!}';
        var DELETE_URL = '{!! url("/powerpanel/service/DeleteRecord") !!}';
        var rollbackRoute = window.site_url + "/powerpanel/service/rollback-record";
        var APPROVE_URL = '{!! url("/powerpanel/service/ApprovedData_Listing") !!}';
        var getChildData = window.site_url + "/powerpanel/service/getChildData";
        var getChildData_rollback = window.site_url + "/powerpanel/service/getChildData_rollback";
        var ApprovedData_Listing = window.site_url + "/powerpanel/service/ApprovedData_Listing";
        var Get_Comments = window.site_url + "/powerpanel/service/Get_Comments";
    ...
    <script src="{{ $CDN_PATH.'resources/pages/scripts/packages/service/service-datatables-ajax.js?v='.time() }}" type="text/javascript"></script>
