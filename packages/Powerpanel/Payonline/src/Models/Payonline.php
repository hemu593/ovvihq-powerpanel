...
class Payonline extends Model {
    ...
    public static function getRecordListDashboard($year = false, $timeparam = false, $month = false) {
        ...
        if ($timeparam != 'month') {
            $response = $response->whereRaw("YEAR(payment_date) = " . (int) $year . "")->count();
        } else {
            $response = $response->whereRaw("YEAR(payment_date) = " . (int) $year . "")->whereRaw("MONTH(payment_date) = " . (int) $month . "")->count();
        }
        return $response;
    }
    ...
    public static function getDashboardReport($year = false) {
        ...
        if ($year != '') {
            $response = $response->whereRaw("YEAR(payment_date) >= " . (int) $year . "");
        }
    ...
    function scopeFilter($query, $filterArr = false, $retunTotalRecords = false) {
        ...
        if (isset($filterArr['searchFilter']) && !empty($filterArr['searchFilter'])) {
            $data = $query->where('name', 'like', '%' . $filterArr['searchFilter'] . '%')->orwhere('companyName', 'like', '%' . $filterArr['searchFilter'] . '%')->orwhere('email', 'like', '%' . $filterArr['searchFilter'] . '%');
        }
        if (!empty($filterArr['rangeFilter']['from']) && $filterArr['rangeFilter']['to']) {
            $data = $query->whereRaw('DATE(payment_date) BETWEEN "' . date('Y-m-d', strtotime(str_replace('/', '-', $filterArr['rangeFilter']['from']))) . '" AND "' . date('Y-m-d', strtotime(str_replace('/', '-', $filterArr['rangeFilter']['to']))) . '"');
        }
