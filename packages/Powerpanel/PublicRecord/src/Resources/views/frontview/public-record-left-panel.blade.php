                ...
                <article>
                    ...
                    <div class="form-group ac-form-group n-mb-0">
                        <select class="selectpicker ac-input" data-width="100%" title="Sort by Category"
                                id="categoryFilter" data-size="5" data-live-search="true">
                            <option value="all">All</option>
                            @if (count($publicRecordCategories) > 0)
                            @foreach ($publicRecordCategories as $category)
                            @php
                            $cont= json_decode($content);
                            @endphp
                            @php $selected = ''; @endphp
                            
                            @if(isset($cont) && isset($cont[0]->val->newscat) && $cont[0]->val->newscat != '')
                                @if($category->id == $cont[0]->val->newscat)
                                    @php $selected = 'selected';  @endphp
                                @endif
                            @endif

                            <option value="{{ $category->id }}" {{ $selected }}>{{ $category->varTitle }}
                            </option>
                            @endforeach
                            @endif
                        </select>
                ...
                <article>
                    ...
                    <div class="form-group ac-form-group n-mb-0">
                        <select class="selectpicker ac-input" data-width="100%" title="Sort by Month"
                                id="monthFilter" data-size="5" data-live-search="true">
