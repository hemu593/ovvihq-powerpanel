                    ...
                    <div class="d-xl-flex flex-wrap align-items-center">
                        <div class="flex-shrink-0 addpage-btn order-sm-2">
                            @can('photo-gallery-create')
                                <a class="btn btn-light btn-theme bg-gradient waves-effect waves-light btn-label" href="{{ url('powerpanel/photo-gallery/add') }}">
                                    <div class="d-flex">
                                        <div class="flex-shrink-0">
                                            <i class="ri-add-line label-icon align-middle fs-20 me-2"></i>
                                        </div>
                                        <div class="flex-grow-1">
                                            {{ trans('photogallery::template.photoGalleryModule.addphotoGallery') }}
                                        </div>
                                    </div>
                                </a>
                            @endcan
                            ...
                            <div class="btn-group d-inline-block filter-dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <div class="p-3 dropdown-body">
                                        ...
                                        <div class="mb-3">
                                            <select class="form-select" name="photoalbumfilter" id="photoalbumfilter">
                                                <option value="">Select Photo Album</option>
                                                @foreach ($photoAlbumsDropDownFilterList as $photoAlbum)
                                                    @php $selected = ''; @endphp
                                                    @if(isset($photoGallery->intFKCategory))
                                                        @if($photoAlbum['id'] == $photoGallery->intFKCategory)
                                                            @php $selected = 'selected'; @endphp
                                                        @endif
                                                    @endif
                                                <option value="{{ $photoAlbum['id'] }}" {{ $selected }} >{{ $photoAlbum['varTitle'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                            ...
                            $tablearray = [
                                'DataTableTab'=>[
                                    'ColumnSetting'=>[
                                        ...
                                        ['Identity_Name'=>'cat','TabIndex'=>'3','Name'=>'Photo Album'],
...
@section('scripts')
    ...
    <script src="{{ $CDN_PATH.'resources/pages/scripts/packages/photogallery/photo-gallery-datatables-ajax.js?v='.time() }}" type="text/javascript"></script>
