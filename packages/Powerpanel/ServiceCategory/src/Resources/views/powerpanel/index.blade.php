    ...
    <div class="col-xl-12">
        ...
        <div class="card">
        	@if($iTotalRecords > 0)
                    ...
                    <div class="d-xl-flex flex-wrap align-items-center">
                        ...
                        <div class="flex-shrink-0 addpage-btn order-sm-2">
                            @can('service-category-create')
                                <a class="btn btn-light btn-theme bg-gradient waves-effect waves-light btn-label" href="{{ url('powerpanel/service-category/add') }}">
                                    <div class="d-flex">
                                        <div class="flex-shrink-0">
                                            <i class="ri-add-line label-icon align-middle fs-20 me-2"></i>
                                        </div>
                                        <div class="flex-grow-1">
                                            {{ trans('servicecategory::template.serviceCategoryModule.addServiceCategory') }}
                                        </div>
                                    </div>
                                </a>
                            @endcan
                                    ...
                                    <div class="p-3 dropdown-body">
                                        <div class="mb-3" id="hidefilter">
                                            <select class="form-select" id="statusfilter">
                                                <option value="">{!! trans('servicecategory::template.common.selectstatus') !!}</option>
                                                <option value="Y">{!! trans('servicecategory::template.common.publish') !!}</option>
                                                <option value="N">{!! trans('servicecategory::template.common.unpublish') !!}</option>
                                            </select>
                                        </div>
                                        @if($userIsAdmin)
                                            <div class="mb-3">
                ...
                <div class="card-body">
                    <div class="live-preview">
                        <div class="table-responsive">
                            ...
                            @endphp
                            @include('powerpanel.partials.datatable-view',['ModuleName'=>'serviceCategory','Permission_Delete'=>'service-category-delete','tablearray'=>$tablearray,'userIsAdmin'=>$userIsAdmin,'Module_ID'=>Config::get('Constant.MODULE.ID')])
                        </div>
                    </div>
                </div><!-- end card-body -->
                ...
                @include('powerpanel.partials.quickeditpopup',['TableName'=>'serviceCategory'])
                @endif
                @else
...
<script type="text/javascript">
    ...
    var rollbackRoute = window.site_url + "/powerpanel/service-category/rollback-record";
    var APPROVE_URL = '{!! url("/powerpanel/service-category/ApprovedData_Listing") !!}';
    var getChildData = window.site_url + "/powerpanel/service-category/getChildData";
    var getChildData_rollback = window.site_url + "/powerpanel/service-category/getChildData_rollback";
    var ApprovedData_Listing = window.site_url + "/powerpanel/service-category/ApprovedData_Listing";
    var Get_Comments = window.site_url + "/powerpanel/service-category/Get_Comments";
...
<script src="{{ $CDN_PATH.'resources/pages/scripts/packages/servicecategory/service-category-datatables-ajax.js?v='.time() }}" type="text/javascript"></script>
...
@if(Auth::user()->hasRole('user_account'))
    <script type="text/javascript">
        ...
        var moduleName = 'ServiceCategory';
    </script>
@endif
@endsection