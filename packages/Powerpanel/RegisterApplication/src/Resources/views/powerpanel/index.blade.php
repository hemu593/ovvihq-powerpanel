            ...
            <div class="card-header border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-shrink-0">
                        @can('register-application-create')
                            @if( isset(App\Helpers\MyLibrary::getFront_Uri('register-application')['uri']) )
                            <a class="btn btn-light bg-gradient waves-effect waves-light btn-label" href="{{ url('powerpanel/register-application/add') }}">
                                <div class="d-flex">
                                    <div class="flex-shrink-0">
                                        <i class="ri-add-line label-icon align-middle fs-20 me-2"></i>
                                    </div>
                                    <div class="flex-grow-1">
                                        {{ trans('register-application::template.registerapplicationModule.addTeamMember') }}
                                    </div>
                                </div>
                            </a>
                            @endif
                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-2 col-sm-4" id="hidefilter">
                        <select class="form-control" id="statusfilter" data-choices data-choices-search-false>
                            <option value="">{!! trans('register-application::template.common.selectstatus') !!}</option>
                            <option value="Y">{!! trans('register-application::template.common.publish') !!}</option>
                            <option value="N">{!! trans('register-application::template.common.unpublish') !!}</option>
                        </select>
                    </div>
            ...
            @elseif($iTotalRecords > 0)
                <div class="card-body">
                    <div class="live-preview">
                        ...
                        $tablearray = [
                            'DataTableTab'=>[
                                'ColumnSetting'=>[
                                    ...
                                    ['Identity_Name'=>'status','TabIndex'=>'3','Name'=>'Status'],
                        ...
                        @endphp
                        @include('powerpanel.partials.datatable-view',['ModuleName'=>'Register of Applications','Permission_Delete'=>'register-application-delete','tablearray'=>$tablearray,'userIsAdmin'=>$userIsAdmin,'Module_ID'=>Config::get('Constant.MODULE.ID')])
                    </div>
                </div><!-- end card-body -->
                <!-- Modal -->
                @else
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/addrecordsection.blade.php') != null)
                @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/register-application/add')])
                @endif
            @endif
        </div><!-- end card -->
    </div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/register-application/DeleteRecord") !!}';
    var APPROVE_URL = '{!! url("/powerpanel/register-application/ApprovedData_Listing") !!}';
    var getChildData = window.site_url + "/powerpanel/register-application/getChildData";
    var getChildData_rollback = window.site_url + "/powerpanel/register-application/getChildData_rollback";
    var ApprovedData_Listing = window.site_url + "/powerpanel/register-application/ApprovedData_Listing";
    var rollbackRoute = window.site_url + "/powerpanel/register-application/rollback-record";
    var Get_Comments = '{!! url("/powerpanel/register-application/Get_Comments") !!}';
...
<script src="{{ $CDN_PATH.'resources/pages/scripts/packages/registerapplication/register-application-datatables-ajax.js' }}" type="text/javascript"></script>
