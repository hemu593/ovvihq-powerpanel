...
/**
 * The RegisterApplication class handels bannner queries
 * ORM implemetation.
...
namespace Powerpanel\RegisterApplication\Models;
...
class RegisterApplication extends Model {
     ...
     * @var array
     */
    protected $table = 'register_application';
    protected $fillable = [
        'id',
        'intAliasId',
        'fkMainRecord',
    ...
    public static function getRecords() {
        $response = false;
        $response = Cache::tags(['RegisterApplication'])->get('getCareersRecords');
        if (empty($response)) {
            $response = Self::Select(['id', 'varTitle','intAliasId', 'varCompanyId', 'varEmail', 'varWeblink1','varWeblink2','varWeblink3','varContactAddress','varStatus','varService', 'varSector',  'intSearchRank', 'txtDescription', 'varContactPerson', 'chrMain', 'chrAddStar', 'intDisplayOrder', 'chrPublish', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'])
                    ->deleted()
                    ->publish()
                    ->get();
            Cache::tags(['RegisterApplication'])->forever('getCareersRecords', $response);
        }
        return $response;
    }
    ...
    public static function getFrontDetail($aliasID) {
        $response = false;
        $response = Cache::tags(['RegisterApplication'])->get('getCareersRecordIdByAliasID_' . $aliasID);
        if (empty($response)) {
            $moduleFields = [
                'id',
                'fkMainRecord',
            ...
            $response = Self::Select($moduleFields)
                    ...
                    ->first();
            Cache::tags(['RegisterApplication'])->forever('getCareersRecordIdByAliasID_' . $aliasID, $response);
        }
        return $response;
    }
    ...
    public static function getAllRegisterApplications($fields = false, $limit, $dbFilter = false)
    {
        $response = false;
        if (empty($response)) {
            $moduleFields = [
            ...
            if(isset($dbFilter['sectorName']) && !empty($dbFilter['sectorName']) ) {
                ...
                $response = $response->where('varSector', '=', $sector_name);
            }
    ...
    public function child() {
		return $this->hasMany('Powerpanel\RegisterApplication\Models\RegisterApplication', 'fkMainRecord', 'id');
	}
    ...
    public static function getChildGrid() {
        ...
        $id = $_REQUEST['id'];
        $response = false;
        $moduleFields = ['id','fkMainRecord', 'varTitle','UserID','intApprovedBy','chrApproved', 'varCompanyId', 'varEmail', 'varWeblink1','varWeblink2','varWeblink3','varContactAddress','varStatus','varService', 'varSector',  'intSearchRank','txtDescription', 'varContactPerson','dtApprovedDateTime', 'chrMain', 'chrAddStar', 'intDisplayOrder', 'chrPublish', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'];
    ...
    public static function getChildrollbackGrid() {
        $id = $_REQUEST['id'];
        $response = false;
        $moduleFields = ['id', 'varTitle', 'varCompanyId', 'varEmail', 'varWeblink1','varWeblink2','varWeblink3','varContactAddress','varStatus','varService', 'varSector',  'intSearchRank','txtDescription', 'varContactPerson', 'chrMain', 'chrAddStar', 'intDisplayOrder', 'chrPublish', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'];
        $response = Self::getPowerPanelRecords($moduleFields, false)
                ->deleted()
                ->where('chrMain', 'N')
                ->where('chrRollBack', 'Y')
    ...
    public static function approved_data_Listing($request) {
        ...
        CommonModel::updateRecords($whereConditions, $updateMainRecord, false, 'Powerpanel\RegisterApplication\Models\RegisterApplication');
...
public static function getBuilderRecordList($filterArr = [])
    {
        $response = false;
        $moduleFields = [
            'register_application.id',
            'register_application.intAliasId',
            'register_application.varTitle',
            'register_application.varSector',
            'register_application.varStatus',
            'register_application.varEmail',
            DB::raw('IFNULL(COUNT(nq_page_hits.id), 0) AS hits'),
            'register_application.chrPublish',
            'register_application.chrDelete',
            'register_application.updated_at',
        ];
        $response = Self::getPowerPanelRecords($moduleFields, false, false, false, false)
            ->filter($filterArr);
        $response = $response->leftJoin('page_hits', 'register_application.intAliasId', '=', 'page_hits.fkIntAliasId')
            ->where('register_application.chrPublish', 'Y')
            ->where('register_application.chrDelete', 'N')
            ->where('register_application.chrMain', 'Y')
            ->where('register_application.chrTrash', '!=', 'Y')
            ->where('register_application.chrDraft', '!=', 'D')
            ->where('register_application.chrIsPreview', 'N')
            ->groupBy('register_application.id')
            ->get();
        return $response;
    }
}
