<?php
namespace Powerpanel\EventCategory\Models;
...
class EventCategory extends Model {
    ...
    public static function getRecordIdByAliasID($aliasID) {
        $response = false;
        $response = Cache::tags(['EventCategory'])->get('getEventCategoryRecordIdByAliasID_' . $aliasID);
        if (empty($response)) {
            $response = Self::Select('id')->deleted()->publish()->checkAliasId($aliasID)->first();
            Cache::tags(['EventCategory'])->forever('getEventCategoryRecordIdByAliasID_' . $aliasID, $response);
        }
        return $response;
    }
    ...
    public static function getRecordDataByAliasID($aliasID) {
        $response = false;
        $response = Cache::tags(['EventCategory'])->get('getPublicationsCatRecordDataByAliasID' . $aliasID);
        if (empty($response)) {
            $response = Self::Select(
                            'id', 'varTitle', 'varSector','intAliasId', 'txtDescription', 'varMetaTitle', 'varMetaKeyword', 'varMetaDescription', 'intSearchRank', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at')
                    ->deleted()
                    ->publish()
                    ...
                    ->first();
            Cache::tags(['EventCategory'])->forever('getPublicationsCatRecordDataByAliasID' . $aliasID, $response);
        }
        return $response;
    }
    public static function getCatWithParent($moduleCode = false,$sector = false) {
        $response = false;
      ...
      public static function getCatDropdown($selected_id = false,$sector) {
          
        $response = false;
        $categoryFields = ['id', 'intAliasId', 'varTitle', 'intDisplayOrder','varSector'];
        $aliasFields = ['id', 'varAlias'];
        $response = Self::getPowerPanelRecords($categoryFields, $aliasFields)
                        ->deleted()
                        ->publish()
                        ->where('chrMain', 'Y')
                        ->where('varSector', $sector)
    ...
    public static function getLatestForHome($limit = 1) {
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['EventCategory'])->get('getLatestForHome_' . $limit);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ...
                    ->get();
            Cache::tags(['EventCategory'])->forever('getLatestForHome_' . $limit, $response);
        }
        return $response;
    }
    /**
     * This method handels retrival of front latest EventCategory list
     * @return  Object
     * @since   2017-10-13
     * @author  NetQuick
     */
    public static function getFrontPopularList($id = false) {
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['EventCategory'])->get('getFrontPopularEventCategoryList_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->take(3)
                    ->get();
            Cache::tags(['EventCategory'])->forever('getFrontPopularEventCategoryList_' . $id, $response);
        }
        return $response;
    }
    ...
    public static function getChildrollbackGrid() {
        $id = $_REQUEST['id'];
        $response = false;
        $cmsPageFields = ['id', 'intAliasId', 'dtDateTime', 'varTitle',  'varSector','txtDescription', 'intDisplayOrder', 'varMetaTitle',
            'varMetaDescription', 'chrPublish', 'chrDelete', 'created_at', 'UserID', 'chrApproved', 'fkMainRecord', 'intApprovedBy', 'intSearchRank',
            'chrPageActive',
            'varPassword',
            'chrDraft',
            'chrTrash',
    ...
    public static function getRecordList_tab1($filterArr = false, $isAdmin = false, $ignoreId = array(), $userRoleSector) {
        $response = false;
        $cmsPageFields = ['id','fkMainRecord', 'intAliasId', 'varTitle', 'varSector', 'dtDateTime', 'txtDescription', 'intDisplayOrder', 'varMetaTitle',
            'varMetaDescription', 'chrPublish', 'chrDelete', 'chrAddStar', 'intSearchRank',
            'chrPageActive',
            'varPassword',
            'chrDraft',
            'chrTrash',
    ...
    /**
     * This method handels retrival of EventCategory records old version *=Delete it afterwards=*
     * @return  Object
     * @since   2016-07-14
     * @author  NetQuick
     */
    ...
    /**
     * This method handels retrival of EventCategory records
     * @return  Object
     * @since   2016-07-14
     * @author  NetQuick
     */
    public static function getFrontRecords($EventCategoryFields = false, $aliasFields = false) {
        $data = [];
        ...
        return self::select($EventCategoryFields)->with($data);
    }
    /**
     * This method handels retrival of EventCategory records
     * @return  Object
    ...
    public function child() {
        return $this->hasMany('Powerpanel\EventCategory\Models\EventCategory', 'fkMainRecord', 'id');
    }
    ...
    public static function getFeaturedList($limit = 10) {
        $response = false;
        $response = Cache::tags(['EventCategory'])->get('getEventCategoryFeaturedList');
        if (empty($response)) {
            ...
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ...
                    ->take($limit)
                    ->get();
            Cache::tags(['EventCategory'])->forever('getEventCategoryFeaturedList', $response);
        }
        return $response;
    }
    /**
    ...
    public static function approved_data_Listing($request) {
        ...
        CommonModel::updateRecords($whereConditions, $updateMainRecord,false,'Powerpanel\EventCategory\Models\EventCategory');
    ...
    public static function getFrontList($filterArr = false, $page = 1, $monthid = false, $yearid = false, $print = false) {
        ...
        $moduleFields = ['id',
            ...
            'updated_at'];
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['EventCategory'])->get('getFrontEventCategoryList_' . $page);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
            ...
            Cache::tags(['EventCategory'])->forever('getFrontEventCategoryList_' . $page, $response);
        }
        return $response;
    }
    ...
    public static function getFrontDetail($id) {
        ...
        $moduleFields = ['id',
            ...
            'updated_at'];
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['EventCategory'])->get('getFrontEventCategoryDetail_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->checkAliasId($id)
                    ->first();
            Cache::tags(['EventCategory'])->forever('getFrontEventCategoryDetail_' . $id, $response);
        }
        return $response;
    }
    /**
    ...
    public static function getLatestList($id = false) {
        ...
        $moduleFields = ['id',
            ...
            'updated_at'];
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['EventCategory'])->get('getFrontLatestEventCategoryList_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->take(5)
                    ->get();
            Cache::tags(['EventCategory'])->forever('getFrontLatestEventCategoryList_' . $id, $response);
        }
        return $response;
    }
    /**
    ...
    public static function getAllCategoriesFrontSidebarList() {
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['EventCategory'])->get('getAllCategoriesFrontSidebarList');
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->where('chrIsPreview', '=', 'N')
                    ->get();
            Cache::tags(['EventCategory'])->forever('getAllCategoriesFrontSidebarList', $response);
        }
        return $response;
    }
    /**
    ...
    public static function getCategorySectorwise($sectorCategory) {
        $response = null;
        $menuFields = ['id', 'varTitle'];
        $response = Self::getRecords($menuFields)
                ->where('chrMain', 'Y')
                ->where('chrIsPreview', 'N')
                ->where('varSector', $sectorCategory)
                ->publish()
                ->deleted()
                ->get();
        return $response;
