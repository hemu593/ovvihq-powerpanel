...
namespace Powerpanel\CareerCategory\Controllers;
...
class CareerCategoryController extends FrontController {
    ...
    public function index($alias = false) {
        ...
        if ($alias) {
            if (is_numeric($alias)) {
                $categoryData = CareerCategory::getRecordById($alias);
            } else {
                $cataliasid = slug::resolve_alias($alias);
                $categoryData = CareerCategory::getRecordDataByAliasID($cataliasid);
            }
        ...
        $careersArr = Careers::getFrontList($currentPage, $limit, $catid, $print, $categoryid, $name, $start_date_time, $end_date_time);
        $careersAllCategoriesArr = CareerCategory::getAllCategoriesFrontSidebarList();
        $data['careersArr'] = $careersArr;
        $data['careersAllCategoriesArr'] = $careersAllCategoriesArr;
        ...
        if ($print == 'print') {
            ...
            $data['currentPage'] = (int) $careersArr->currentPage();
            $data['lastPage'] = (int) $careersArr->lastPage();
            $data['modulePageUrl'] = $moduelFrontPageUrl;
            $data['moduleFrontWithCatUrl'] = $moduleFrontWithCatUrl;
        }
        ...
        if (Request::ajax()) {
            ...
            $returnHtml = view('careercategory::frontview.careerscategory', $data)->render();
            $returnRepsonse = array('html' => $returnHtml, 'lastpage' => $data['lastPage'], 'currentpage' => $data['currentPage']);
            echo json_encode($returnRepsonse);
        } else {
            return view('careercategory::frontview.careerscategory', $data);
        }
    }
