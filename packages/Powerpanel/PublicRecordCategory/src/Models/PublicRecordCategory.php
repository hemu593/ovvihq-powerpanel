...
namespace Powerpanel\PublicRecordCategory\Models;
...
class PublicRecordCategory extends Model {

    protected $table = 'public_record_category';
    protected $fillable = [
        'id',
        'varTitle','varSector',
        'intAliasId',
    ...
    public static function getRecordIdByAliasID($aliasID) {
        $response = false;
        $response = Cache::tags(['PublicRecordCategory'])->get('getPublicationsCatRecordIdByAliasID' . $aliasID);
        if (empty($response)) {
            $response = Self::Select('id', 'varTitle','varSector', 'intAliasId')
                    ...
                    ->first();
            Cache::tags(['PublicRecordCategory'])->forever('getPublicationsCatRecordIdByAliasID' . $aliasID, $response);
        }
        return $response;
    }
    ...
    public static function getRecordDataByAliasID($aliasID) {
        $response = false;
        $response = Cache::tags(['PublicRecordCategory'])->get('getPublicationsCatRecordDataByAliasID' . $aliasID);
        if (empty($response)) {
            $response = Self::Select('id', 'varTitle','varSector', 'intAliasId', 'varMetaTitle', 'varMetaDescription', 'chrPageActive', 'varPassword','UserID', 'chrDraft', 'intSearchRank', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at')
                    ...
                    ->checkAliasId($aliasID)
                    ->first();
            Cache::tags(['PublicRecordCategory'])->forever('getPublicationsCatRecordDataByAliasID' . $aliasID, $response);
        }
        return $response;
    }
    ...
    public static function getFrontList() {
        $response = false;
        $publicrecordCategoryFields = ['id', 'varTitle'];
        $response = Cache::tags(['PublicRecordCategory'])->get('getFrontServiceCatList');
        if (empty($response)) {
            $response = Self::getFrontRecords($publicrecordCategoryFields)
                    ->deleted()
                    ->publish()
                    ->get()
                    ->pluck('varTitle','varSector', 'id');
            Cache::tags(['PublicRecordCategory'])->forever('getFrontServiceCatList', $response);
        }
        return $response;
    }
     ...
     * @author  NetQuick
     */
    public function public_record_category() {
        $response = false;
        $response = $this->hasOne('App\PublicRecordCategory', 'id', 'intParentCategoryId');
        return $response;
    }
    ...
    public function parentCategory() {
        $response = false;
        $response = $this->belongsTo('App\PublicRecordCategory', 'intParentCategoryId', 'id');
        return $response;
    }
    ...
    public function child() {
		return $this->hasMany('Powerpanel\PublicRecordCategory\Models\PublicRecordCategory', 'fkMainRecord', 'id');
	}
     ...
     * @author  NetQuick
     */
    public static function getRecordListforPublicRecordCategoryGrid($filterArr = false, $checkMain = true, $isAdmin = false, $userRoleSector) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleFields = [
            'id',
     ...
     * @author  NetQuick
     */
    public static function getDraftRecordListforPublicRecordCategoryGrid($filterArr = false, $checkMain = true, $isAdmin = false, $userRoleSector) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleFields = [
            'id',
     ...
     * @author  NetQuick
     */
    public static function getFavoriteRecordListforPublicRecordCategoryGrid($filterArr = false, $checkMain = true, $isAdmin = false, $userRoleSector) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleFields = [
            'id',
     ...
     * @author  NetQuick
     */
    public static function getTrashRecordListforPublicRecordCategoryGrid($filterArr = false, $checkMain = true, $isAdmin = false, $userRoleSector) {
        $userid = auth()->user()->id;
        $response = false;
        $moduleFields = [
            'id',
    ...
    public static function getRecordListforGridbyIds($ids, $filterArr = false) {
        $userid = auth()->user()->id;
        $response = false;
        $response = DB::table('public_record_category AS Og')
                ->leftjoin('public_record_category as Og1', 'Og.intParentCategoryId', '=', 'Og1.id')
    ...
    public static function getParentNodesIdsByRecordId($recordId = false) {
        $response = array();
        if ($recordId) {
            $tree = DB::select("select GetAncestry_PublicRecordCategory(?) AS tree", [$recordId]);
    ...
    public static function getCategoryList($paginate = 6, $page) {
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['PublicRecordCategory'])->get('getCategoryList_' . $page);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ...
                    ->checkMainRecord('Y')
                    ->paginate($paginate);
            Cache::tags(['PublicRecordCategory'])->forever('getCategoryList_' . $page, $response);
        }
        return $response;
    }
    ...
    public static function getAllCategoriesFrontSidebarList() {
        ...
        $moduleFields = ['id', 'intAliasId', 'varTitle', 'varSector', 'intParentCategoryId', 'intDisplayOrder', 'chrPageActive', 'varPassword', 'chrDraft', 'intSearchRank', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'];
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['PublicRecordCategory'])->get('getAllCategoriesFrontSidebarList');
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->orderBy('intDisplayOrder', 'ASC')
                    ->get();
            Cache::tags(['PublicRecordCategory'])->forever('getAllCategoriesFrontSidebarList', $response);
        }
        return $response;
    }
    ...
    public static function getFrontDetail($id) {
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['PublicRecordCategory'])->get('getPublicRecordCategoryFrontDetail_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->dateRange()
                    ...
                    ->checkAliasId($id)
                    ->first();
            Cache::tags(['PublicRecordCategory'])->forever('getPublicRecordCategoryFrontDetail_' . $id, $response);
        }
        return $response;
    }
    ...
    public static function UpdateDisplayOrder($order, $parentRecordId = false) {
        $updateSql = "UPDATE nq_public_record_category SET `intDisplayOrder` = `intDisplayOrder` + 1
    ...
    public static function updateherarachyRecords($when, $ids) {
        $response = false;
        $response = DB::statement("UPDATE nq_public_record_category SET intDisplayOrder = (CASE id " . $when . " ELSE intDisplayOrder END) WHERE id BETWEEN " . min($ids) . " AND " . max($ids) . " and chrDelete = 'N' and chrMain='Y'");
        return $response;
    }

    public static function updateDisplayOrderByParent($id, $newOrder, $parentRecordId) {
        $response = false;
        $updateQuery = "UPDATE nq_public_record_category SET intDisplayOrder=intDisplayOrder + 1 WHERE intDisplayOrder >= " . $newOrder . " and chrDelete = 'N' AND id!=$id AND chrDelete = 'N' AND intParentCategoryId=$parentRecordId";
    ...
    public static function approved_data_Listing($request) {
        ...
        CommonModel::updateRecords($whereConditions, $updateMainRecord, false, 'Powerpanel\PublicRecordCategory\Models\PublicRecordCategory');
        ...
        CommonModel::updateRecords($whereConditions_ApproveN, $updateToApproveN, false, 'Powerpanel\PublicRecordCategory\Models\PublicRecordCategory');
        ...
        CommonModel::updateRecords($whereConditionsApprove, $updateToApprove, false, 'Powerpanel\PublicRecordCategory\Models\PublicRecordCategory');
    ...
    public static function getPublicRecordCategoryNameByPublicRecordCategoryId($ids) {
        $response = false;
