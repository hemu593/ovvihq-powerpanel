            ...
            <div class="card-header border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-shrink-0">
                        @can('complaint-services-create')
                            @if( isset(App\Helpers\MyLibrary::getFront_Uri('complaint-services')['uri']) )
                            <a class="btn btn-light bg-gradient waves-effect waves-light btn-label" href="{{ url('powerpanel/complaint-services/add') }}">
                                <div class="d-flex">
                                    <div class="flex-shrink-0">
                                        <i class="ri-add-line label-icon align-middle fs-20 me-2"></i>
                                    </div>
                                    <div class="flex-grow-1">
                                        {{ trans('complaint-services::template.complaintserviceModule.addcomplaintservice') }}
                                    </div>
                                </div>
                            </a>
                            @endif
                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-2 col-sm-4" id="hidefilter">
                        <select class="form-control" id="statusfilter" data-choices data-choices-search-false>
                            <option value="">{!! trans('complaint-services::template.common.selectstatus') !!}</option>
                            <option value="Y">{!! trans('complaint-services::template.common.publish') !!}</option>
                            <option value="N">{!! trans('complaint-services::template.common.unpublish') !!}</option>
                        </select>
                    </div>
                    @if($userIsAdmin)
                        <div class="col-lg-2 col-sm-4">
            ...
            @elseif($iTotalRecords > 0)
                <div class="card-body">
                    <div class="live-preview">
                        ...
                        @endphp
                        @include('powerpanel.partials.datatable-view',['ModuleName'=>'Complaint Services','Permission_Delete'=>'complaint-services-delete','tablearray'=>$tablearray,'userIsAdmin'=>$userIsAdmin,'Module_ID'=>Config::get('Constant.MODULE.ID')])
                    </div>
                </div><!-- end card-body -->
                <!-- Modal -->
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/quickeditpopup.blade.php') != null)
                @include('powerpanel.partials.quickeditpopup',['TableName'=>'complaint-services'])
                @endif
                @else
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/addrecordsection.blade.php') != null)
                @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/complaint-services/add')])
                @endif
            @endif
        </div><!-- end card -->
    </div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/complaint-services/DeleteRecord") !!}';
    var APPROVE_URL = '{!! url("/powerpanel/complaint-services/ApprovedData_Listing") !!}';
    var getChildData = window.site_url + "/powerpanel/complaint-services/getChildData";
    var getChildData_rollback = window.site_url + "/powerpanel/complaint-services/getChildData_rollback";
    var ApprovedData_Listing = window.site_url + "/powerpanel/complaint-services/ApprovedData_Listing";
    var rollbackRoute = window.site_url + "/powerpanel/complaint-services/rollback-record";
    var Get_Comments = '{!! url("/powerpanel/complaint-services/Get_Comments") !!}';
...
<script src="{{ $CDN_PATH.'resources/pages/scripts/packages/complaintservices/complaint-datatables-ajax.js' }}" type="text/javascript"></script>
