...
public function set_image_html()
{
	...
	<p>Image file must be on png, jpg, jpeg, and gif format.</p>
	</div>
	</div>
	</div>
	...
	<div class="form-group cm-floating">
		...
		<div class="error help-block mb-0"></div>
	</div>
	</div>
	<div class="col-md-12">
...
public function set_video_html()
{
	...
	<div class="dz-message needsclick">
	<div class="dropzone_icon">
	<i class="display-4 text-muted ri-upload-cloud-2-fill icons"></i>
	</div>
...
{
	$response = ["status" => false, 'message' => 'Oops! file not found'];
	...
	if (Request::file('file') || null !== Request::file('upload')) {
		...
		if (exif_imagetype($file->getPathName())) {
			...
			if (isset($_REQUEST['folderid']) && $_REQUEST['folderid'] != '') {
				...
				if ($this->BUCKET_ENABLED) {
					if (!empty($folderdata)) {
						$folderPath = $this->S3_MEDIA_BUCKET_PATH . '/' . $folderdata->foldername . '/';
					} else {
						$folderPath = $this->S3_MEDIA_BUCKET_PATH . '/';
					}
					Aws_File_helper::putObject($sourceFilePath, $folderPath, $storeFileName);
				} else {
			...
			$response = ["status" => true,'imageID' => $imageID];

		} else {
			$response = ["status" => false, 'message' => "You can't upload files of this type"];
		}
	}
...
public function upload_documents()
{
	...
	if (Request::file('file')) {
		...
		if (in_array(strtolower($pathinfo['extension']), $allowedExtentions)) {
			$timestamp = str_replace([' ', ':'], '-', Carbon::now()->toDateTimeString());
			$name = $timestamp . '-' . self::clean($pathinfo['filename']);
			$storeFileName = $name . '.' . $pathinfo['extension'];
			...
			if (isset($_REQUEST['folderid']) && $_REQUEST['folderid'] != '') {
				...
				if ($this->BUCKET_ENABLED) {
					if (!empty($folderdata)) {
						$folderPath = $this->S3_MEDIA_BUCKET_DOCUMENT_PATH . '/' . $folderdata->foldername . '/';
					} else {
						$folderPath = $this->S3_MEDIA_BUCKET_DOCUMENT_PATH . '/';
					}
					Aws_File_helper::putObject($sourceFilePath, $folderPath, $storeFileName);
				} else {
			...
			$response = ['status' => true, 'documentID' => $documentID];                        

		} else {
			$response = ['status' => false, 'message' => 'Please upload valid file with valid extention and mime type.'];
		}
	}
...
public function upload_audios()
{
	...
	if (Request::file('file')) {
		...
		if (isset($_REQUEST['folderid']) && $_REQUEST['folderid'] != '') {
			...
			if ($this->BUCKET_ENABLED) {
				if (!empty($folderdata)) {
					$folderPath = $this->S3_MEDIA_BUCKET_AUDIO_PATH . '/' . $folderdata->foldername . '/';
				} else {
					$folderPath = $this->S3_MEDIA_BUCKET_AUDIO_PATH . '/';
				}
				Aws_File_helper::putObject($sourceFilePath, $folderPath, $storeFileName);
			} else {
...
public function folder_uploaded_image()
{
	...
	if (Request::get('userid')) {
		...
		$filterArr['imageName'] = $imageName;
		$images = Image::getImages($limit, $page, 0, $filterArr);
		$Image_html = $this->getFolderImageHtml($images, 'Folders', $filterArr, $limit);
		$response['Image_html'] = $Image_html;
		$response['imageCount'] = count($images);
	}
	...
	if ($more_document->count() > 0) {
		foreach ($more_document as $key => $value) {
			...
			if ($this->filePathExist($documentPath)) {
				...
				<input class='form-control' type='text' name='documentname" . $value->id . "' id='documentname_" . $value->id . "' value='" . $value->txtDocumentName . "'/><a onclick=\"MediaManager.GetUpdateDocumentName('" . $value->id . "')\" href=\"javascript:void(0);\" class='btn'><i class='ri-pencil-line'></i></a>
				</div>
  			...
  			$docsHtml .= "<div class='img-box contains_thumb' id='document_" . $value->id . "'>
  				<div class='thumbnail_container'>
  				<div class='thumbnail'>
  				<a  title='Document not available' href='javascript:void(0);'>";
  				$docsHtml .= "<img alt='" . $value->txtDocumentName . "' src='" . $this->_APP_URL . "assets/images/documents_logo/no_doc.png'>";
  				$docsHtml .= "<span class='icon-check' aria-hidden='true'></span>
  				</a>
  				</div>
  				</div>
  				<div class='title-change'>
  				<input class='form-control' type='text' name='documentname" . $value->id . "' id='documentname_" . $value->id . "' readonly value='" . $value->txtDocumentName . "'/>
  				</div>
  				<a class='right_check' href='javascript:void(0)' ><i class=''></i></a>
  				</div>";
  		}
	...
	if ($more_images->count() > 0) {
		...
		foreach ($more_images as $key => $value) {
			...
			if ($this->filePathExist($img_path)) {
				...
				$Image_html .= "<div class='img-btns'><a class='right_check' href='javascript:void(0)' ><i class=''></i></a>";
				...
				if ($value->varImageExtension != "svg") {
					$Image_html .= "<a href='javascript:;' title='Crop Image' onclick=\"MediaManager.cropImage('" . $value->id . "');\"><span class='ri-crop-line'></span></a>";
				}
...
public function folder_uploaded_docs()
{
	...
	if (Request::get('userid')) {
		...
		$documentObj = Document::getDistinctFolderDocuments($limit, $page, 0, $filterArr);
		$Doc_html = $this->getFolderDocHtml($documentObj, 'Folders', $filterArr, $limit);
		$response['Doc_html'] = $Doc_html;
		$response['docCount'] = count($documentObj);
	}
        //echo $response;
...
public function getImageDetails()
{
	...
	if ($imageObj->varfolder == 'folder') {
		...
		if ($this->BUCKET_ENABLED) {
			$imagePath = $this->S3_MEDIA_BUCKET_PATH . '/' .$folderObj->foldername. '/' . $imageObj->txtImageName . '.' . $imageObj->varImageExtension;
		}else{
			$imagePath = Config::get('Constant.LOCAL_CDN_PATH').'/assets/images/' .$folderObj->foldername. '/' . $imageObj->txtImageName . '.' . $imageObj->varImageExtension;
		}

		$mimeType = mime_content_type($imagePath);
		$dimensionArr = getimagesize($imagePath);
		...
		$file = filesize($imagePath);
		$fileSize = Self::formatSizeUnits($file);
	} else {

		if ($this->BUCKET_ENABLED) {
			$imagePath = $this->S3_MEDIA_BUCKET_PATH . '/' . $imageObj->txtImageName . '.' . $imageObj->varImageExtension;
		}else{
			$imagePath = Config::get('Constant.LOCAL_CDN_PATH').'/assets/images/'.$imageObj->txtImageName . '.' . $imageObj->varImageExtension;
		}
...
public function cropImage()
{
	...
	if ($this->BUCKET_ENABLED) {
		if ($imageObj->varfolder == 'folder') {
			$imageURL = $this->_APP_URL.$this->S3_MEDIA_BUCKET_PATH.'/'.$folderObj->foldername . '/' . $imageObj->txtImageName . '.' . $imageObj->varImageExtension;
		} else {
			$imageURL = $this->_APP_URL.$this->S3_MEDIA_BUCKET_PATH.'/'.$imageObj->txtImageName . '.' . $imageObj->varImageExtension;
		}
	}else{
		if ($imageObj->varfolder == 'folder') {
...
public function saveCroppedImage() {
	...
	if (!empty($imageObj)) {
		...
		if ($overwrite == 'true') {
			...
			if ($this->BUCKET_ENABLED) {
				if ($imageObj->varfolder == 'folder') {
					$path = $this->S3_MEDIA_BUCKET_PATH.'/'.$folderObj->foldername.'/'.$name . '.' . $extension;
					$savePath = $this->S3_MEDIA_BUCKET_PATH.'/'.$folderObj->foldername;
				} else {
					$path = $this->S3_MEDIA_BUCKET_PATH.'/'.$name . '.' . $extension;
					$savePath = $this->S3_MEDIA_BUCKET_PATH;
				}
			}else{
				if ($imageObj->varfolder == 'folder') {
			...
			if ($this->BUCKET_ENABLED) {
				Aws_File_helper::putObject($path, $savePath.'/', $name.'.'.$extension);
			}
...
public function folder_uploaded_audios()
{
	...
	if (Request::get('userid')) {
		...
		$audioumentObj = Audio::getAudios($limit, $page, 0, $filterArr);
		$Doc_html = $this->getFolderAudioHtml($audioumentObj, 'Folders', $filterArr, $limit);
		$response['Doc_html'] = $Doc_html;
		$response['audioCount'] = count($audioumentObj);
	}
        //echo $response;
...
public static function clean($string)
{
    	...
    	if (count($audioObj) > 0) {
    		...
    		foreach ($audioObj as $key => $value) {
    			...
    			if ($this->filePathExist($audioPath)) {
    				...
    				<div class='title-change'>
    				<input class='form-control' type='text' name='audioname" . $value->id . "' id='audioname_" . $value->id . "' value='" . $value->txtAudioName . "'/><a onclick=\"MediaManager.GetUpdateAudioName('" . $value->id . "')\" href=\"javascript:void(0);\" class='btn'><i class='ri-pencil-line'></i></a>
    	...
    	if ($imageObj->count() > 0) {
    		...
    		foreach ($imageObj as $key => $value) {
    			...
    			if ($this->filePathExist($img_path)) {
    				...
    				$Image_html .= "<div class='img-btns'><a class='right_check text_doc' href='javascript:void(0)' ><i class=''></i></a>";
                    // $Image_html .= "<a href='javascript:;' title='Image Detail' onclick=\"MediaManager.getImageDetails('" . $value->id . "');\"><span class='icon icon-info'></span></a>";
    ...
    public function folderdata()
    {
    	...
    	$folder_name = Self::clean($record['name']);

    	$folderObj = DB::table('folder')
    	->where('type',$record['type'])
    	->where('foldername', $folder_name)
    	->first();

    	if (empty($folderObj)) {

    		$folderID = DB::table('folder')->insertGetId(
    			['type' => $record['type'], 'foldername' => $folder_name, 'date' => date('Y-m-d H:i:s')]
    		);
    		if ($record['type'] == 1) {
    			if ($this->BUCKET_ENABLED) {
    				$path = $this->S3_MEDIA_BUCKET_PATH . '/assets/images/' . $folder_name;
    			} else {
    				$path = Config::get('Constant.LOCAL_CDN_PATH') . '/assets/images/' . $folder_name;
    			}

    		} else if ($record['type'] == 2) {

    			if ($this->BUCKET_ENABLED) {
    				$path = $this->S3_MEDIA_BUCKET_PATH . '/documents/' . $folder_name;
    			} else {
    				$path = Config::get('Constant.LOCAL_CDN_PATH') . '/documents/' . $folder_name;
    			}
    		} else if ($record['type'] == 3) {
    			if ($this->BUCKET_ENABLED) {
    				$path = $this->S3_MEDIA_BUCKET_PATH . '/audios/' . $folder_name;
    			} else {
    				$path = Config::get('Constant.LOCAL_CDN_PATH') . '/audios/' . $folder_name;
    			}
    		}
    		...
    		$folderList = DB::table('folder')
    		->where('type', '=', $record['type'])
    		->get();

    		$html .= '<option value="0">All</option>';
    		foreach ($folderList as $fdata) {
    			$selected = '';
    			if ($folderID == $fdata->id) {
    				$selected = 'selected';
    			}
    			$html .= '<option value="' . $fdata->id . '" selected="' . $selected . '">' . $fdata->foldername . '</option>';
    		}
    		$html .= '</select>';
    		...
    		if(count($folderList) > 0) {
    			foreach ($folderList as $fdata) {
    				$selected = '';
    				if ($folderObj->id == $fdata->id) {
    					$selected = 'selected';
    				}
    				$html .= '<option value="' . $fdata->id . '" ' . $selected . '>' . $fdata->foldername . '</option>';
    			}
    		}
    		$html .= '</select>';

    		$response['success'] = 500;
    		$response['folder_id'] = $folderObj->id;
    		$response['html'] = $html;
    		$response['type'] = $record['type'];
    	}
        ...
        //                 } else {
        //                     $path = Config::get('Constant.LOCAL_CDN_PATH').'/assets/images/' . $record['name'];
        //                 }
        ...
        //                 } else {
        //                     $path = Config::get('Constant.LOCAL_CDN_PATH').'/documents/' . $record['name'];
        //                 }
        //             } else if ($record['type'] == 3) {
        //                 if ($this->BUCKET_ENABLED) {
        //                     $path = $this->S3_MEDIA_BUCKET_PATH . '/audios/' . $record['name'];
        //                 } else {
        //                     $path = Config::get('Constant.LOCAL_CDN_PATH').'/audios/' . $record['name'];
        //                 }
        //             }
        //             if (!File::isDirectory($path)) {
        //                 File::makeDirectory($path, 0777, true, true);
    ...
    public function GetFolderImages()
    {
    	...
    	foreach ($folder_data as $fdata) {
    		...
    		<div class=\"img-btns\"><a class=\"right_check text_doc\" href=\"javascript:void(0)\"><i class=\"" . $iconclass . "\"></i></a><a href=\"javascript:;\" title=\"Crop Image\" onclick=\"MediaManager.cropImage('" . $fdata->id . "','" . $fdata->fk_folder . "');\"><span class=\"ri-crop-line\"></span></a></div>
    		</div>";
    		$i++;
    	}
    	echo $html;
