    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {// here you can define a typical datatable settings from http://datatables.net/usage/options
                ...
                "ajax": {
                    "url": window.site_url + "/powerpanel/tag/get_list", // ajax source
                },
        ...
        $(document).on('keyup', '#searchfilter', function (e) {
            ...
            if (action.length >= 2) {
                $.cookie('tagsearch', action);
                grid.setAjaxParam("customActionType", "group_action");
                grid.setAjaxParam("searchValue", action);
                grid.setAjaxParam("id", grid.getSelectedRows());
                grid.getDataTable().ajax.reload();
            } else {
                $.removeCookie('tagsearch');
                grid.setAjaxParam("customActionType", "group_action");
                grid.setAjaxParam("searchValue", action);
                grid.setAjaxParam("id", grid.getSelectedRows());
                grid.getDataTable().ajax.reload();
    ...
    var handleRecords1 = function () {
        ...
        grid1.init({
            ...
            dataTable: {// here you can define a typical datatable settings from http://datatables.net/usage/options
                ...
                "ajax": {
                    "url": window.site_url + "/powerpanel/tag/get_list_New", // ajax source
                },
                'fnCreatedRow': function (nRow, aData, iDataIndex) {
...
$(window).on('load', function () {
    if ($.cookie('tagsearch')) {
        $('#searchfilter').val($.cookie('tagsearch'));
...
function reorder(curOrder, excOrder) {
    var ajaxurl = site_url + '/powerpanel/tag/reorder';
    $.ajax({
        url: ajaxurl,
        data: {
            order: curOrder,
