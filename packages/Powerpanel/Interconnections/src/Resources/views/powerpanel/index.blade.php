            ...
            <div class="card-header border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-shrink-0">
                        @can('interconnections-create')
                            <a class="btn btn-light bg-gradient waves-effect waves-light btn-label" href="{{ url('powerpanel/interconnections/add') }}">
                                <div class="d-flex">
                                    <div class="flex-shrink-0">
                                        <i class="ri-add-line label-icon align-middle fs-20 me-2"></i>
                                    </div>
                                    <div class="flex-grow-1">
                                        {{ trans('interconnections::template.interconnectionsModule.addInterconnections') }}
                                    </div>
                                </div>
                            </a>
                        @endcan
                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-2 col-sm-4" id="hidefilter">
                        <select class="form-control" id="statusfilter" data-choices data-choices-search-false>
                            <option value="">{!! trans('interconnections::template.common.selectstatus') !!}</option>
                            <option value="Y">{!! trans('interconnections::template.common.publish') !!}</option>
                            <option value="N">{!! trans('interconnections::template.common.unpublish') !!}</option>
                        </select>
                    </div>
                    ...
                    <div class="col-lg-4 col-sm-6" id="hidefilter">
                        <select class="form-control category_filter" name="category" id="categoryfilter" data-choices data-choices-search-false>
                            <option value="">Select Category</option>
                            @if(!empty($ParentsRecords))
                                @foreach ($ParentsRecords as $cat)
                                <option value="{{ $cat['id'] }}" >{{ $cat['varTitle'] }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
            ...
            @if($iTotalRecords > 0)
                <div class="card-body">
                    <div class="live-preview">
                        ...
                        @include('powerpanel.partials.tabpanel',['tabarray'=>['trashTotalRecords', 'draftTotalRecords', 'favoriteTotalRecords']])
                        @endif
                        @php
                        $tablearray = [
                            'DataTableTab'=>[
                                'ColumnSetting'=>[
                                    ['Identity_Name'=>'name','TabIndex'=>'2','Name'=>'Category Title'],
                                    ['Identity_Name'=>'pinterconnection','TabIndex'=>'3','Name'=>'Parent InterConnection'],
                                ...
                                'DataTableHead'=>[
                                    ['Title'=>'Category Title','Align'=>'left'],
                                    ['Title'=>'Parent InterConnection','Align'=>'left'],
                        ...
                        @endphp
                        @include('powerpanel.partials.datatable-view',['ModuleName'=>'Interconnections','Permission_Delete'=>'interconnections-delete','tablearray'=>$tablearray,'userIsAdmin'=>$userIsAdmin,'Module_ID'=>Config::get('Constant.MODULE.ID')])
                    </div>
                </div><!-- end card-body -->
                <!-- Modal -->
                @else
                @if (File::exists(base_path() . '/resources/views/powerpanel/partials/addrecordsection.blade.php') != null)
                @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/interconnections/add')])
                @endif
            @endif
        </div><!-- end card -->
    </div>
...
<script type="text/javascript">
	window.site_url =  '{!! url("/") !!}';
	var DELETE_URL =  '{!! url("/powerpanel/interconnections/DeleteRecord") !!}';
	var APPROVE_URL = '{!! url("/powerpanel/interconnections/ApprovedData_Listing") !!}';
	var getChildData = window.site_url + "/powerpanel/interconnections/getChildData";
	var getChildData_rollback = window.site_url +"/powerpanel/interconnections/getChildData_rollback";
	var ApprovedData_Listing = window.site_url +"/powerpanel/interconnections/ApprovedData_Listing";
    var rollbackRoute = window.site_url + "/powerpanel/interconnections/rollback-record";
	var Get_Comments = window.site_url + "/powerpanel/interconnections/Get_Comments";
	...
	var interconnectionsData = jQuery.parseJSON('{!!$intdata!!}');
</script>
...
<script src="{{ $CDN_PATH.'resources/pages/scripts/packages/interconnections/interconnections-datatables-ajax.js' }}" type="text/javascript"></script>
