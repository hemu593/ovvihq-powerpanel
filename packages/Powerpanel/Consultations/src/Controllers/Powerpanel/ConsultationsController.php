...
namespace Powerpanel\Consultations\Controllers\Powerpanel;
...
class ConsultationsController extends PowerpanelController
{
    ...
    /**
     * This method handels load process of Consultations
     * @return  View
     * @since   2017-11-10
     * @author  NetQuick
     */
    public function index()
    {
        ...
        $iTotalRecords = Consultations::getRecordCount(false, false, $userIsAdmin, $this->currentUserRoleSector);
        $draftTotalRecords = Consultations::getRecordCountforListDarft(false, true, $userIsAdmin, array(), $this->currentUserRoleSector);
        $trashTotalRecords = Consultations::getRecordCountforListTrash(false, false, $userIsAdmin, array(), $this->currentUserRoleSector);
        $favoriteTotalRecords = Consultations::getRecordCountforListFavorite(false, false, $userIsAdmin, array(), $this->currentUserRoleSector);
        $NewRecordsCount = Consultations::getNewRecordsCount($userIsAdmin, $this->currentUserRoleSector);

        $categories = array('consultations' => 'Consultations', 'determinations' => 'Determinations', 'completed_consultations' => 'Completed Consultations');
        $this->breadcrumb['title'] = trans('consultations::template.consultationsModule.manageConsultations');
        ...
        return view('consultations::powerpanel.index', compact('iTotalRecords', 'breadcrumb', 'NewRecordsCount', 'userIsAdmin', 'categories', 'draftTotalRecords', 'trashTotalRecords', 'favoriteTotalRecords', 'settingarray'));
    }

    /**
     * This method loads Consultations edit view
    ...
    public function edit($id = false)
    {
        ...
        if (!is_numeric($id)) {
            $total = Consultations::getRecordCount(false, false, $userIsAdmin, $this->currentUserRoleSector);
            if (auth()->user()->can('consultations-create') || $userIsAdmin) {
                $total = $total + 1;
            }
            $categories = array('consultations' => 'Consultations', 'determinations' => 'Determinations', 'completed_consultations' => 'Completed Consultations');

            $this->breadcrumb['title'] = trans('consultations::template.consultationsModule.addConsultations');
            $this->breadcrumb['module'] = trans('consultations::template.consultationsModule.manageConsultations');
            $this->breadcrumb['url'] = 'powerpanel/consultations';
            $this->breadcrumb['inner_title'] = trans('consultations::template.consultationsModule.addConsultations');
            $breadcrumb = $this->breadcrumb;
            $data = compact('documentManager', 'breadcrumb', 'imageManager', 'videoManager', 'userIsAdmin', 'categories', 'total');
        } else {
            $documentManager = true;
            $consultations = Consultations::getRecordById($id);
            if (empty($consultations)) {
                return redirect()->route('powerpanel.consultations.add');
            }
            $categories = array('consultations' => 'Consultations', 'determinations' => 'Determinations', 'completed_consultations' => 'Completed Consultations');
            if ($consultations->fkMainRecord != '0') {
                $consultations_highLight = Consultations::getRecordById($consultations->fkMainRecord);
                $templateData['consultations_highLight'] = $consultations_highLight;
                $metaInfo_highLight['varMetaTitle'] = $consultations_highLight['varMetaTitle'];
                $metaInfo_highLight['varMetaDescription'] = $consultations_highLight['varMetaDescription'];
                $metaInfo_highLight['varTags'] = $consultations_highLight['varTags'];
                $display_publish = $consultations_highLight['chrPublish'];
            } else {
                $consultations_highLight = "";
                $templateData['consultations_highLight'] = "";
                $metaInfo_highLight['varMetaTitle'] = "";
                $metaInfo_highLight['varMetaDescription'] = "";
                $metaInfo_highLight['varTags'] = "";
                $display_publish = '';
            }
            $metaInfo = array('varMetaTitle' => $consultations->varMetaTitle,
                'varMetaDescription' => $consultations->varMetaDescription,
                'varTags' => $consultations->varTags,
            );
            if (method_exists($this->MyLibrary, 'getModulePageAliasByModuleName')) {
                $categorypagereocrdlink = MyLibrary::getModulePageAliasByModuleName('consultations');
            }
            // if (method_exists($this->MyLibrary, 'getRecordAliasByModuleNameRecordId')) {
            //     $categoryRecordAlias = MyLibrary::getRecordAliasByModuleNameRecordId("news-category", $consultations->txtCategories);
            // }
            if (!empty($categorypagereocrdlink)) {
                $varURL = $categorypagereocrdlink . '/' . $consultations->alias->varAlias;
            } else {
                $varURL = $consultations->alias->varAlias;
            }
            $metaInfo['varURL'] = $varURL;
            $this->breadcrumb['title'] = trans('consultations::template.consultationsModule.editConsultations') . ' - ' . $consultations->varTitle;
            $this->breadcrumb['module'] = trans('consultations::template.consultationsModule.manageConsultations');
            $this->breadcrumb['url'] = 'powerpanel/consultations';
            $this->breadcrumb['inner_title'] = trans('consultations::template.consultationsModule.editConsultations') . ' - ' . $consultations->varTitle;
            $breadcrumb = $this->breadcrumb;
            $data = compact('consultations', 'documentManager', 'metaInfo', 'breadcrumb', 'imageManager', 'videoManager', 'consultations_highLight', 'metaInfo_highLight', 'display_publish', 'userIsAdmin', 'categories');
        }
        ...
        return view('consultations::powerpanel.actions', $data);
    }

    /**
     * This method stores Consultations modifications
     * @return  View
     * @since   2017-11-10
     * @author  NetQuick
     */
    public function handlePost(Request $request)
    {
        ...
        $actionMessage = trans('consultations::template.common.oppsSomethingWrong');
        $messsages = array(
            'title.required' => 'Title field is required.',
            'sector.required' => 'Sector field is required.',
            'short_description.required' => trans('consultations::template.consultationsModule.shortDescription'),
            'category_id.required' => trans('consultations::template.consultationsModule.categoryMessage'),
            'varMetaTitle.required' => trans('consultations::template.consultationsModule.metaTitle'),
            'varMetaDescription.required' => trans('consultations::template.consultationsModule.metaDescription'),
        );
        ...
        if ($validator->passes()) {
            ...
            $consultationsArr = [];
            $consultationsArr['varTitle'] = stripslashes(trim($data['title']));
            $consultationsArr['fkIntDocId'] = !empty($data['doc_id']) ? $data['doc_id'] : null;
            $consultationsArr['dtDateTime'] = !empty($data['start_date_time']) ? date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $data['start_date_time']))) : date('Y-m-d H:i:s');
            $consultationsArr['dtEndDateTime'] = !empty($data['end_date_time']) ? date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $data['end_date_time']))) : null;
            $consultationsArr['txtDescription'] = $vsection;
            $consultationsArr['varShortDescription'] = $data['short_description'];
            $consultationsArr['varMetaTitle'] = stripslashes(trim($data['varMetaTitle']));
            $consultationsArr['varMetaDescription'] = stripslashes(trim($data['varMetaDescription']));
            $consultationsArr['varTags'] = trim($data['tags']);
            $consultationsArr['intDisplayOrder'] = trim($data['display_order']);

            $consultationsArr['UserID'] = auth()->user()->id;
            if ($data['chrMenuDisplay'] == 'D') {
                $consultationsArr['chrDraft'] = 'D';
                $consultationsArr['chrPublish'] = 'N';
            } else {
                $consultationsArr['chrDraft'] = 'N';
                $consultationsArr['chrPublish'] = $data['chrMenuDisplay'];
            }
            if (isset($data['chrPageActive']) && $data['chrPageActive'] != '') {
                $consultationsArr['chrPageActive'] = $data['chrPageActive'];
            }
            if (isset($data['chrPageActive']) && $data['chrPageActive'] == 'PP') {
                $consultationsArr['varPassword'] = $data['new_password'];
            } else {
                $consultationsArr['varPassword'] = '';
            }
            if ($data['chrMenuDisplay'] == 'D') {
            ...
            if (Config::get('Constant.CHRSearchRank') == 'Y') {
                $consultationsArr['intSearchRank'] = $data['search_rank'];
            }
            $consultationsArr['txtCategories'] = isset($data['category_id']) ? $data['category_id'] : null;
            $id = Request::segment(3);
            if (is_numeric($id)) {
                if ($data['oldAlias'] != $data['alias']) {
                    Alias::updateAlias($data['oldAlias'], $data['alias']);
                }
                $consultations = Consultations::getRecordForLogById($id);
                $whereConditions = ['id' => $consultations->id];
                $consultationsArr['varSector'] = $data['sector'];
                if ($consultations->chrLock == 'Y' && auth()->user()->id != $consultations->LockUserID) {
                    if ($this->currentUserRoleData->chrIsAdmin != 'Y') {
                        $lockedUserData = User::getRecordById($consultations->LockUserID, true);
                        ...
                        return redirect()->route('powerpanel.consultations.index')->with('message', $actionMessage);
                    }
                }
                if (File::exists(app_path() . '/Workflow.php') != null || File::exists(base_path() . '/packages/Powerpanel/Workflow/src/Models/Workflow.php') != null) {
                    if (!$userIsAdmin) {
                        ...
                        $userRoleData = Role_user::getUserRoleByUserId($consultations->UserID);
                    ...
                    if (empty($workFlowByCat->varUserId) || $userIsAdmin || $workFlowByCat->charNeedApproval == 'N') {
                        if ($consultations->fkMainRecord == '0' || empty($workFlowByCat->varUserId)) {
                            $update = CommonModel::updateRecords($whereConditions, $consultationsArr, false, 'Powerpanel\Consultations\Models\Consultations');
                            if ($update) {
                                if (!empty($id)) {
                                    $logArr = MyLibrary::logData($consultations->id, false, $addlog);
                                    if (Auth::user()->can('log-advanced')) {
                                        $newConsultationsObj = Consultations::getRecordForLogById($consultations->id);
                                        $oldRec = $this->recordHistory($consultations);
                                        $newRec = $this->newrecordHistory($consultations, $newConsultationsObj);
                                    ...
                                    if (Auth::user()->can('recent-updates-list')) {
                                        if (!isset($newConsultationsObj)) {
                                            $newConsultationsObj = Consultations::getRecordForLogById($consultations->id);
                                        }
                                        $notificationArr = MyLibrary::notificationData($consultations->id, $newConsultationsObj);
                                        RecentUpdates::setNotification($notificationArr);
                                    }
                                    self::flushCache();
                                    if (isset($data['saveandexit']) && $data['saveandexit'] == 'approvesaveandexit') {
                                        $actionMessage = trans('consultations::template.common.recordApprovalMessage');
                                    } else {
                                        $actionMessage = trans('consultations::template.consultationsModule.updateMessage');
                                    }
                                }
                            }
                        } else {
                            $updateModuleFields = $consultationsArr;
                        ...
                        if ($workFlowByCat->charNeedApproval == 'Y') {
                            $approvalObj = $this->insertApprovalRecord($consultations, $data, $consultationsArr);
                            if (isset($data['saveandexit']) && $data['saveandexit'] == 'approvesaveandexit') {
                                $actionMessage = trans('consultations::template.common.recordApprovalMessage');
                            } else {
                                $actionMessage = trans('consultations::template.consultationsModule.updateMessage');
            ...
            if (File::exists(app_path() . '/Workflow.php') != null || File::exists(base_path() . '/packages/Powerpanel/Workflow/src/Models/Workflow.php') != null) {
                ...
                if (!empty($workFlowByCat->varUserId) && $workFlowByCat->chrNeedAddPermission == 'Y' && !$userIsAdmin) {
                    $consultationsArr['chrPublish'] = 'N';
                    $consultationsArr['chrDraft'] = 'N';
                    $consultationsObj = $this->insertNewRecord($data, $consultationsArr);
                    if ($data['chrMenuDisplay'] == 'D') {
                        $consultationsArr['chrDraft'] = 'D';
                    }
                    $consultationsArr['chrPublish'] = 'Y';
                    $approvalObj = $this->insertApprovalRecord($consultationsObj, $data, $consultationsArr);
                    $approval = $consultationsObj->id;
                } else {
                    $consultationsObj = $this->insertNewRecord($data, $consultationsArr);
                    $approval = $consultationsObj->id;
                }
                if (isset($data['saveandexit']) && $data['saveandexit'] == 'approvesaveandexit') {
                    ...
                    $actionMessage = trans('consultations::template.consultationsModule.addMessage');
                }
                $id = $consultationsObj->id;
            }
            ...
            if ((!empty($request->saveandexit) && $request->saveandexit == 'saveandexit') || !$userIsAdmin) {
                if ($data['chrMenuDisplay'] == 'D') {
                    return redirect()->route('powerpanel.consultations.index', 'tab=D')->with('message', $actionMessage);
                } else {
                    return redirect()->route('powerpanel.consultations.index')->with('message', $actionMessage);
                }
            } else {
                return redirect()->route('powerpanel.consultations.edit', $id)->with('message', $actionMessage);
            }
        } else {
    ...
    public function insertApprovedRecord($updateModuleFields, $postArr, $id)
    {
        ...
        $update = CommonModel::updateRecords($whereConditions, $updateModuleFields, false, 'Powerpanel\Consultations\Models\Consultations');
        // if ($update) {
        ...
        $newCmsPageObj = Consultations::getRecordForLogById($id);
        ...
        $careers = Consultations::getRecordForLogById($id);
    ...
    public function insertApprovalRecord($moduleObj, $postArr, $consultationsArr)
    {
        $response = false;
        $consultationsArr['intAliasId'] = MyLibrary::insertAlias($postArr['alias']);
        $consultationsArr['chrMain'] = 'N';
        $consultationsArr['chrLetest'] = 'Y';
        $consultationsArr['fkMainRecord'] = $moduleObj->id;
        $consultationsArr['varSector'] = $postArr['sector'];
        $consultationsArr['varTags'] = trim($postArr['tags']);
        if ($postArr['chrMenuDisplay'] == 'D') {
            $consultationsArr['chrDraft'] = 'D';
            $consultationsArr['chrPublish'] = 'N';
        } else {
            $consultationsArr['chrDraft'] = 'N';
            $consultationsArr['chrPublish'] = $postArr['chrMenuDisplay'];
        }
        if (isset($postArr['chrPageActive']) && $postArr['chrPageActive'] != '') {
            $consultationsArr['chrPageActive'] = $postArr['chrPageActive'];
        }
        if (isset($postArr['chrPageActive']) && $postArr['chrPageActive'] == 'PP') {
            $consultationsArr['varPassword'] = $postArr['new_password'];
        } else {
            $consultationsArr['varPassword'] = '';
        }
        if (Config::get('Constant.CHRSearchRank') == 'Y') {
            $consultationsArr['intSearchRank'] = $postArr['search_rank'];
        }
        if ($postArr['chrMenuDisplay'] == 'D') {
        ...
        $consultationsID = CommonModel::addRecord($consultationsArr, 'Powerpanel\Consultations\Models\Consultations');
        if (!empty($consultationsID)) {
            $id = $consultationsID;
            ...
            $newConsultationsObj = Consultations::getRecordForLogById($id);
            $logArr = MyLibrary::logData($id, false, $addlog);
            $logArr['varTitle'] = $newConsultationsObj->varTitle;
            Log::recordLog($logArr);
            if (Auth::user()->can('recent-updates-list')) {
                $notificationArr = MyLibrary::notificationData($id, $newConsultationsObj);
                RecentUpdates::setNotification($notificationArr);
            }
            $response = $newConsultationsObj;
            self::flushCache();
            $actionMessage = trans('consultations::template.consultationsModule.addMessage');
        }
        ...
        CommonModel::updateRecords($whereConditionsAddstar, $updateAddStar, false, 'Powerpanel\Consultations\Models\Consultations');
        return $response;
    }

    public function insertNewRecord($postArr, $consultationsArr)
    {
        $response = false;
        $consultationsArr['varSector'] = $postArr['sector'];
        $consultationsArr['intAliasId'] = MyLibrary::insertAlias($postArr['alias']);
        $consultationsArr['chrMain'] = 'Y';
        $consultationsArr['varTags'] = trim($postArr['tags']);
        ...
        if (!empty($consultationsID)) {
            ...
            $newConsultationsObj = Consultations::getRecordForLogById($id);
            $logArr = MyLibrary::logData($id, false, $addlog);
            $logArr['varTitle'] = stripslashes($newConsultationsObj->varTitle);
            Log::recordLog($logArr);
            if (Auth::user()->can('recent-updates-list')) {
                $notificationArr = MyLibrary::notificationData($id, $newConsultationsObj);
                RecentUpdates::setNotification($notificationArr);
    ...
    /**
     * This method loads consultations table data on view
     * @return  View
     * @since   2017-11-10
     * @author  NetQuick
     */
    public function get_list_New()
    {
        ...
        $arrResults = Consultations::getRecordList_tab1($filterArr, $isAdmin, $this->currentUserRoleSector);
        $iTotalRecords = Consultations::getRecordCountListApprovalTab($filterArr, $isAdmin, $this->currentUserRoleSector);
        $end = $filterArr['iDisplayStart'] + $filterArr['iDisplayLength'];
        $end = $end > $iTotalRecords ? $iTotalRecords : $end;
        $totalRecords = Consultations::getRecordCount(false, false, $isAdmin, $this->currentUserRoleSector);
        if (!empty($arrResults)) {
            foreach ($arrResults as $key => $value) {
                $records["data"][] = $this->tableData_tab1($value, $totalRecords);
            }
        }
        $NewRecordsCount = Consultations::getNewRecordsCount($isAdmin, $this->currentUserRoleSector);
    ...
    public function get_list()
    {
        ...
        $arrResults = Consultations::getRecordList($filterArr, $isAdmin, $this->currentUserRoleSector);
        $iTotalRecords = Consultations::getRecordCountforList($filterArr, true, $isAdmin, $this->currentUserRoleSector);
        $end = $filterArr['iDisplayStart'] + $filterArr['iDisplayLength'];
        $end = $end > $iTotalRecords ? $iTotalRecords : $end;
        $totalRecords = Consultations::getRecordCount(false, false, $isAdmin, $this->currentUserRoleSector);
        if (!empty($arrResults)) {
            foreach ($arrResults as $key => $value) {
                $records["data"][] = $this->tableData($value, false, $totalRecords);
            }
        }
        $NewRecordsCount = Consultations::getNewRecordsCount($isAdmin, $this->currentUserRoleSector);
        $records["newRecordCount"] = $NewRecordsCount;
    ...
    public function get_list_favorite()
    {
        ...
        $arrResults = Consultations::getRecordListFavorite($filterArr, $isAdmin, $this->currentUserRoleSector);
        $iTotalRecords = Consultations::getRecordCountforListFavorite($filterArr, true, $isAdmin, array(), $this->currentUserRoleSector);
        $totalRecords = Consultations::getRecordCount(false, false, $isAdmin, $this->currentUserRoleSector);
        $end = $filterArr['iDisplayStart'] + $filterArr['iDisplayLength'];
        $end = $end > $iTotalRecords ? $iTotalRecords : $end;
        if (!empty($arrResults)) {
            foreach ($arrResults as $key => $value) {
                $records["data"][] = $this->tableDataFavorite($value, $cmsPageForModule, $totalRecords);
            }
        }
        $NewRecordsCount = Consultations::getNewRecordsCount($isAdmin, $this->currentUserRoleSector);
        $records["newRecordCount"] = $NewRecordsCount;
    ...
    public function get_list_draft()
    {
        ...
        $module = Modules::getModule('pages');
        $cmsPageForModule = CmsPage::getRecordForPowerpanelShareByModuleId(Config::get('Constant.MODULE.ID'), $module->id);
        $arrResults = Consultations::getRecordListDraft($filterArr, $isAdmin, $this->currentUserRoleSector);
        $iTotalRecords = Consultations::getRecordCountforListDarft($filterArr, true, $isAdmin, array(), $this->currentUserRoleSector);
        $totalRecords = Consultations::getRecordCount(false, false, $isAdmin, $this->currentUserRoleSector);
        $end = $filterArr['iDisplayStart'] + $filterArr['iDisplayLength'];
        $end = $end > $iTotalRecords ? $iTotalRecords : $end;
        // $totalRecords = Consultations::getRecordCount(false, false, $isAdmin, $this->currentUserRoleSector);
        if (!empty($arrResults)) {
            foreach ($arrResults as $key => $value) {
                $records["data"][] = $this->tableDataDraft($value, $cmsPageForModule, $totalRecords);
            }
        }
        $NewRecordsCount = Consultations::getNewRecordsCount($isAdmin, $this->currentUserRoleSector);
        $records["newRecordCount"] = $NewRecordsCount;
    ...
    public function get_list_trash()
    {
        ...
        $module = Modules::getModule('pages');
        $cmsPageForModule = CmsPage::getRecordForPowerpanelShareByModuleId(Config::get('Constant.MODULE.ID'), $module->id);
        $arrResults = Consultations::getRecordListTrash($filterArr, $isAdmin, $this->currentUserRoleSector);
        $iTotalRecords = Consultations::getRecordCountforListTrash($filterArr, true, $isAdmin, array(), $this->currentUserRoleSector);
        $totalRecords = Consultations::getRecordCount(false, false, $isAdmin, $this->currentUserRoleSector);
        $end = $filterArr['iDisplayStart'] + $filterArr['iDisplayLength'];
        $end = $end > $iTotalRecords ? $iTotalRecords : $end;
        $totalRecords = Consultations::getRecordCount(false, false, $isAdmin, $this->currentUserRoleSector);
        if (!empty($arrResults)) {
            foreach ($arrResults as $key => $value) {
                $records["data"][] = $this->tableDataTrash($value, $cmsPageForModule, $totalRecords);
            }
        }
        $NewRecordsCount = Consultations::getNewRecordsCount($isAdmin, $this->currentUserRoleSector);
        $records["newRecordCount"] = $NewRecordsCount;
    ...
    public function tableData($value = false, $moduleCmsPageData = false, $totalRecord = false)
    {
        $sector = '';
        ...
        if (Auth::user()->can('consultations-edit')) {
            $details .= '<a class="" title="' . trans("consultations::template.common.edit") . '" href="' . route('powerpanel.consultations.edit', array('alias' => $value->id)) . '"><i class="ri-pencil-line"></i></a>';
        }
        if (Auth::user()->can('consultations-delete') || (isset($this->currentUserRoleData->chrIsAdmin) && $this->currentUserRoleData->chrIsAdmin == 'Y')) {
            if (Config::get('Constant.DEFAULT_TRASH') == 'Y') {
                $details .= '<a class="delete-grid" title="' . trans("consultations::template.common.delete") . '" onclick = \'Trashfun("' . $value->id . '")\' data-controller="ConsultationsController" data-alias = "' . $value->id . '" data-tab="P"><i class="ri-delete-bin-line"></i></a>';
            } else {
                $details .= '<a class="delete" title="' . trans("consultations::template.common.delete") . '" data-controller="ConsultationsController" data-alias = "' . $value->id . '" data-tab="P"><i class="ri-delete-bin-line"></i></a>';
            }
        }
        if ($value->chrAddStar != 'Y') {
            if ($value->chrDraft != 'D') {
                if (!empty($value->chrPublish) && ($value->chrPublish == 'Y')) {
                    //Bootstrap Switch
                    $publish_action .= view('powerpanel.partials.bootstrap-switch', ['data_controller'=>'powerpanel/consultations', 'data_alias'=>$value->id, 'title'=>trans("consultations::template.common.publishedRecord"), 'data_value'=>'Unpublish'])->render();
                } else {
                    //Bootstrap Switch
                    $publish_action .= view('powerpanel.partials.bootstrap-switch', ['data_controller'=>'powerpanel/consultations', 'data_alias'=>$value->id, 'title'=>trans("consultations::template.common.unpublishedRecord"), 'data_value'=>'Publish', 'checked'=>'checked'])->render();
                }
        ...
        if (isset($this->currentUserRoleData->chrIsAdmin) && $this->currentUserRoleData->chrIsAdmin == 'Y') {
            // $details .= '<a class=" share" title="Share" data-modal="consultations" data-alias="' . $value->id . '"  data-images="" data-link = "' . url('/' . $value->alias['varAlias']) . '" data-toggle="modal" data-target="#confirm_share">
        ...
        if (isset($value->txtCategories)) {
            ...
            $categories = array('consultations' => 'Consultations', 'determinations' => 'Determinations', 'completed_consultations' => 'Completed Consultations');
            // $selCategory = $categories[$categoryIDs];
            $category = $categories[$categoryIDs];
            // $category .= '<div class="pro-act-btn"><a href="javascript:void(0)" class="" onclick="return hs.htmlExpand(this,{width:300,headingText:\'' . trans("consultations::template.common.category") . '\',wrapperClassName:\'titlebar\',showCredits:false});"><span aria-hidden="true" class="icon-info"></span></a>';
            // $category .= '<div class="highslide-maincontent">';
            // $category .= '<ul><li>' . $selCategory . '</li>';
            // $category .= '<ul>';
            // $category .= '</div>';
            // $category .= '</div>';
        } else {
        ...
        if (Auth::user()->can('consultations-reviewchanges') && (File::exists(app_path() . '/Workflow.php') != null || File::exists(base_path() . '/packages/Powerpanel/Workflow/src/Models/Workflow.php') != null)) {
        ...
        // $endDateFormat = date('' . Config::get('Constant.DEFAULT_DATE_FORMAT') . ' ', strtotime($value->dtEndDateTime));
        // $endDate = !empty($endDateFormat) ? $endDateFormat : 'No Expiry';

        $endDate = !empty($value->dtEndDateTime) ? date('' . Config::get('Constant.DEFAULT_DATE_FORMAT') . ' ', strtotime($value->dtEndDateTime)) : 'No Expiry';

        $Quickedit_startDate = date('Y-m-d', strtotime($value->dtDateTime));
        $Quickedit_endDate = !empty($value->dtEndDateTime) ? date('Y-m-d', strtotime($value->dtEndDateTime)) : 'No Expiry';

        $title = $value->varTitle;

        if (Auth::user()->can('consultations-edit')) {
            if ($value->chrDraft == 'D' || $value->chrAddStar == 'Y') {
                $viewlink = url('/previewpage?url=' . MyLibrary::getFrontUri('consultations')['uri'] . '/' . $value->id . '/preview/detail');
                $linkviewLable = "Preview";
            } else {
                $viewlink = MyLibrary::getFrontUri('consultations')['uri'] . '/' . $value->alias->varAlias;
                $linkviewLable = "View";
            }
            if ($value->chrLock != 'Y') {
                if (isset($this->currentUserRoleData->chrIsAdmin) && $this->currentUserRoleData->chrIsAdmin == 'Y') {
                    $title = '<div class="quick_edit"><a href = "' . route('powerpanel.consultations.edit', array('alias' => $value->id)) . '?tab=P">' . $value->varTitle . '</a> <div class="quick_edit_menu">
														<span><a href="' . route('powerpanel.consultations.edit', array('alias' => $value->id)) . '?tab=P" title="Edit">Edit</a></span>';
    ...
    public function tableDataDraft($value = false, $moduleCmsPageData = false, $totalRecord = false)
    {
        $sector = '';
        if (isset($value->varSector) && !empty($value->varSector)) {
            $sector = strtoupper($value->varSector);
        ...
        if (Auth::user()->can('consultations-delete') || (isset($this->currentUserRoleData->chrIsAdmin) && $this->currentUserRoleData->chrIsAdmin == 'Y')) {
            if (Config::get('Constant.DEFAULT_TRASH') == 'Y') {
                $details .= '<a class="delete-grid" title="' . trans("consultations::template.common.delete") . '" onclick = \'Trashfun("' . $value->id . '")\' data-controller="consultations" data-alias = "' . $value->id . '" data-tab="D"><i class="ri-delete-bin-line"></i></a>';
            } else {
                $details .= '<a class="delete" title="' . trans("consultations::template.common.delete") . '" data-controller="consultations" data-alias = "' . $value->id . '" data-tab="D"><i class="ri-delete-bin-line"></i></a>';
            }
        }
        ...
        $startDate = date('Y-m-d', strtotime($value->dtDateTime));
        $endDate = !empty($value->dtEndDateTime) ? date('Y-m-d', strtotime($value->dtEndDateTime)) : 'No Expiry';
    ...
    public function tableDataFavorite($value = false, $moduleCmsPageData = false, $totalRecord = false)
    {
        $sector = '';
        if (isset($value->varSector) && !empty($value->varSector)) {
            $sector = strtoupper($value->varSector);
        ...
        if ($value->chrAddStar != 'Y') {
            if ($value->chrDraft != 'D') {
                if (Auth::user()->can('consultations-publish')) {
                    if (!empty($value->chrPublish) && ($value->chrPublish == 'Y')) {
                        //Bootstrap Switch
                        $publish_action .= view('powerpanel.partials.bootstrap-switch', ['data_controller'=>'powerpanel/consultations', 'data_alias'=>$value->id, 'title'=>trans("consultations::template.common.publishedRecord"), 'data_value'=>'Unpublish'])->render();
                    } else {
    ...
    public function tableDataTrash($value = false, $moduleCmsPageData = false, $totalRecord = false)
    {
        $sector = '';
        if (isset($value->varSector) && !empty($value->varSector)) {
            $sector = strtoupper($value->varSector);
        ...
        $details = '';
        $publish_action = '';
        if (Auth::user()->can('consultations-delete') && $this->currentUserRoleData->chrIsAdmin == 'Y') {
            $details .= '<a class="delete" title="' . trans("consultations::template.common.delete") . '" data-controller="consultations" data-alias = "' . $value->id . '" data-tab="T"><i class="ri-delete-bin-line"></i></a>';
        }
        ...
        $endDate = !empty($value->dtEndDateTime) ? date('Y-m-d', strtotime($value->dtEndDateTime)) : 'No Expiry';
        $title = $value->varTitle;
        $title = '<div class="quick_edit text-uppercase"><a href = "' . route('powerpanel.consultations.edit', array('alias' => $value->id)) . '?tab=T">' . $value->varTitle . '</a>
        </div>';
        $logurl = url('powerpanel/log?id=' . $value->id . '&mid=' . Config::get('Constant.MODULE.ID'));
        if ($details == "") {
            $details = "---";
    ...
    public function tableData_tab1($value = false, $totalRecord = false)
    {
        $sector = '';
        if (isset($value->varSector) && !empty($value->varSector)) {
            $sector = strtoupper($value->varSector);
    ...
    /**
     * This method delete multiples Consultations
     * @return  true/false
     * @since   2017-07-15
     * @author  NetQuick
     */
    ...
    public function DeleteRecord(Request $request)
    {
        ...
        $update = MyLibrary::deleteMultipleRecords($data, $moduleHaveFields, $value, 'Powerpanel\Consultations\Models\Consultations');
        ...
        foreach ($update as $ids) {
            ...
            $Deleted_Record = Consultations::getRecordById($ids, $ignoreDeleteScope);
            $Cnt_Letest = Consultations::getRecordCount_letest($Deleted_Record['fkMainRecord'], $Deleted_Record['id']);
            if ($Cnt_Letest <= 0) {
                $updateLetest = [
                    'chrAddStar' => 'N',
                ];
                $whereConditionsApprove = ['id' => $Deleted_Record['fkMainRecord']];
                CommonModel::updateRecords($whereConditionsApprove, $updateLetest, false, 'Powerpanel\Consultations\Models\Consultations');
                if (File::exists(app_path() . '/Workflow.php') != null || File::exists(base_path() . '/packages/Powerpanel/Workflow/src/Models/Workflow.php') != null) {
                    $where = [];
                    $flowData = [];
    ...
    public function reorder()
    {
        ...
        MyLibrary::swapOrder($order, $exOrder, 'Powerpanel\Consultations\Models\Consultations');
        self::flushCache();
    }
    ...
    public static function swap_order_add($order = null)
    {
        ...
        if ($order != null) {
            $response = MyLibrary::swapOrderAdd($order, $isCustomizeModule, false, 'Powerpanel\Consultations\Models\Consultations');
            self::flushCache();
        }
        return $response;
    }
    ...
    {
        $isCustomizeModule = true;
        MyLibrary::swapOrderEdit($order, $id, $isCustomizeModule, 'Powerpanel\Consultations\Models\Consultations');
        self::flushCache();
    }
    ...
    public function publish(Request $request)
    {
        ...
        $update = MyLibrary::setPublishUnpublish($alias, $val, 'Powerpanel\Consultations\Models\Consultations');
        self::flushCache();
        echo json_encode($update);
        exit;
    }
    ...
    public function recordHistory($data = false)
    {
        ...
        $categoryIDs = $data->txtCategories;
        $categories = array('consultations' => 'Consultations', 'determinations' => 'Determinations', 'completed_consultations' => 'Completed Consultations');
        $selCategory = $categories[$categoryIDs];
        ...
        $returnHtml .= '<table class="new_table_desing table table-striped table-bordered table-hover">
                            <thead>
                                    <tr>
                                        <th align="center">' . trans("Consultations::template.common.title") . '</th>
                                        ...
                                        <th align="center">' . trans("Consultations::template.common.publish") . '</th>
                                    </tr>
                            </thead>
                            <tbody>
                                    <tr>
                                        <td align="center">' . stripslashes($data->varTitle) . '</td>
                                        <td align="center">' . $selCategory . '</td>
        ...
        $returnHtml .= '<table class="new_table_desing table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                    <tr>
                                        ...
                                        <td align="center" ' . $catcolor . '>' . $selCategory . '</td>
    ...
    public function addPreview(Guard $auth)
    {
        ...
        $consultationsArr['varMetaDescription'] = stripslashes(trim($data['varMetaDescription']));
        $consultationsArr['chrPublish'] = $data['chrMenuDisplay'];
        $consultationsArr['chrIsPreview'] = 'Y';
        $consultationsArr['varTags'] = trim($data['tags']);
        $consultationsArr['txtCategories'] = isset($data['category_id']) ? $data['category_id'] : null;
        ...
        if ($data['oldAlias'] != $data['alias']) {
            ...
            if ($update) {
                if (!empty($id)) {
                    $logArr = MyLibrary::logData($consultations->id);
                    if (Auth::user()->can('log-advanced')) {
                        $newConsultationsObj = Consultations::getRecordForLogById($consultations->id);
                        $oldRec = $this->recordHistory($consultations);
                        $newRec = $this->recordHistory($newConsultationsObj);
                        $logArr['old_val'] = $oldRec;
                        $logArr['new_val'] = $newRec;
                    }
        ...
        $consultationsArr['fkIntDocId'] = !empty($data['doc_id']) ? $data['doc_id'] : null;
            $consultationsArr['intAliasId'] = MyLibrary::insertAlias($data['alias'], false, 'Y');
            $id = CommonModel::addRecord($consultationsArr, 'Powerpanel\Consultations\Models\Consultations');
        }
        return json_encode(array('status' => $id, 'alias' => $data['alias'], 'message' => trans('consultations::template.pageModule.pageUpdate')));
    }
    ...
    public function getChildData()
    {
        $childHtml = "";
        $Cmspage_childData = "";
        $Cmspage_childData = Consultations::getChildGrid();
        ...
        if (count($Cmspage_childData) > 0) {
            foreach ($Cmspage_childData as $child_row) {
                ...
                // $categoryRecordAlias = MyLibrary::getRecordAliasByModuleNameRecordId("news-category", $child_row->txtCategories);
                $previewlink = url('/previewpage?url=' . MyLibrary::getFrontUri('consultations')['uri'] . '/' . $child_row->id . '/preview/detail');
                ...
                if ($child_row->chrApproved == 'N') {
                    $childHtml .= "<td class=\"text-center\"><span class='mob_show_title'>Edit: </span><a class='icon_round' title='" . trans("consultations::template.common.edit") . "' href='" . route('powerpanel.consultations.edit', array('alias' => $child_row->id)) . "'>
							<i class='ri-pencil-line'></i></a></td>";
                } else {
                    $childHtml .= "<td class=\"text-center\"><span class='mob_show_title'>Edit: </span>-</td>";
                }
                if ($child_row->chrApproved == 'N') {
                    $childHtml .= "<td class=\"text-center\"><span class='mob_show_title'>Status: </span><a class=\"approve_icon_btn\" title='" . trans("consultations::template.common.comments") . "' href=\"javascript:;\" onclick=\"loadModelpopup('" . $child_row->id . "','" . $child_row->UserID . "','" . Config::get('Constant.MODULE.MODEL_NAME') . "','" . $child_row->fkMainRecord . "')\"><i class=\"ri-chat-1-line\"></i> <span>Comment</span></a>    <a  class=\"approve_icon_btn\" onclick=\"update_mainrecord('" . $child_row->id . "','" . $child_row->fkMainRecord . "','" . $child_row->UserID . "','A');\" title='" . trans("consultations::template.common.clickapprove") . "' href=\"javascript:;\"><i class=\"ri-checkbox-line\"></i> <span>Approve</span></a></td>";
                } else {
    ...
    public function getChildData_rollback()
    {
        ...
        $Cmspage_rollbackchildData = Consultations::getChildrollbackGrid();
        ...
        if (count($Cmspage_rollbackchildData) > 0) {
            foreach ($Cmspage_rollbackchildData as $child_rollbacrow) {
                ...
                $previewlink = url('/previewpage?url=' . MyLibrary::getFrontUri('consultations')['uri'] . '/' . $child_rollbacrow->id . '/preview/detail');
    ...
    public function ApprovedData_Listing(Request $request)
    {
        ...
        $message = Consultations::approved_data_Listing($request);
        $newCmsPageObj = Consultations::getRecordForLogById($main_id);
        $approval_obj = Consultations::getRecordForLogById($approvalid);
        if ($flag == 'R') {
    ...
    public function get_buider_list()
    {
        ...
        $arrResults = Consultations::getBuilderRecordList($filterArr);
        ...
        $iTotalRecords = CommonModel::getTotalRecordCount('Powerpanel\Consultations\Models\Consultations', true, true);
        $records["data"] = $rows;
    ...
    public function rollBackRecord(Request $request)
    {
        ...
        $previousRecord = Consultations::getPreviousRecordByMainId($request->id);
        if (!empty($previousRecord)) {
            ...
            /* notification for user to record approved */
            $blogs = Consultations::getRecordForLogById($previousRecord->id);
            if(!empty($blogs))
            {
                if (method_exists($this->MyLibrary, 'userNotificationData')) {
                    $userNotificationArr = MyLibrary::userNotificationData(Config::get('Constant.MODULE.ID'));
            ...
            /* notification for user to record approved */
            $newBlogObj = Consultations::getRecordForLogById($main_id);
            $restoredata = Config::get('Constant.ROLLBACK_RECORD');
            if(!empty($newBlogObj)) {
                $logArr = MyLibrary::logData($main_id, false, $restoredata);
