...
<div class="row n-mt-25 justify-content-center">
    @foreach($data['fmbroadcasting'] as $key =>  $fmbroadcast)
    ...
    $moduleFrontWithCatUrl = ($fmbroadcast->varAlias != false ) ? $moduelFrontPageUrl . '/' . $fmbroadcast->varAlias : $moduelFrontPageUrl;
    
    $recordLinkUrl = $moduleFrontWithCatUrl.'/'.$fmbroadcast->alias->varAlias;
    } else {
    $recordLinkUrl = '';
    }
    @endphp
    ...
    @if(isset($fmbroadcast->fkIntImgId))
    @php                          
    $itemImg = App\Helpers\resize_image::resize($fmbroadcast->fkIntImgId);
    @endphp
    @else 
    @php
    ...
    $description = '';
    @endphp
    @if(isset($fmbroadcast->varShortDescription))
    @php
    $description = $fmbroadcast->varShortDescription;
    @endphp
    ...
    <div class="d-flex col-lg-3 col-md-4 col-6 n-gapp-lg-5 n-gapm-lg-4 n-gapm-md-3" data-aos="zoom-in" data-aos-delay="{{$key}}00">
        <article class="-items n-bs-1 n-bgc-white-500 w-100">
            <div class="thumbnail-container ac-webp" data-thumb="100%">
                ...
                <div class="-freq" data-aos="fade-right" data-aos-delay="{{$key}}00">{{$fmbroadcast->txtFrequency}}</div>
                <div class="-play"><a href="{{$fmbroadcast->varLink}}" title="Play" rel="nofollow" target="_blank" class="n-ti-05"><i class="n-icon" data-icon="s-play-circle"></i></a></div>
            </div>
            <div class="n-pa-20">
                <div class="-title n-ti-05 n-fs-18 n-fs-22-sm n-fw-500 n-fc-dark-500 n-lh-110">{{$fmbroadcast->varTitle}}</div>
