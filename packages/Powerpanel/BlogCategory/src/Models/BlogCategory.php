...
class BlogCategory extends Model {
    ...
    public static function getRecordList_tab1($filterArr = false, $isAdmin = false, $ignoreId = array(), $userRoleSector) {
        $response = false;
        $cmsPageFields = ['id', 'intAliasId', 'varTitle','varSector', 'dtDateTime', 'txtDescription', 'intDisplayOrder', 'varMetaTitle', 'varMetaDescription', 'chrPublish', 'chrDelete','LockUserID','chrLock', 'chrAddStar', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'];
        $aliasFields = ['id', 'varAlias'];
        $MainIDs = Self::distinct()
                ->select("fkMainRecord")
                ->where('chrMain', 'N')
    ...
    public function child() {
		return $this->hasMany('Powerpanel\BlogCategory\Models\BlogCategory', 'fkMainRecord', 'id');
	}
    /**
     * This method handels retrival of BlogCategory records
     * @return  Object
