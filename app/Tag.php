<?php
/**
 * The tag class handels bannner queries
 * ORM implemetation.
...
class Tag extends Model {
    ...
    /**
     * This method handels retrival of tags records
     * @return  Object
     * @since   2016-07-20
     * @author  NetQuick
     */
    ...
    public static function getRecords() {
        $response = false;
        $response = Cache::tags(['tag'])->get('gettagRecords');
        if (empty($response)) {
            $response = Self::Select(['id', 'varTitle', 'intDisplayOrder', 'chrPublish'])
                    ->deleted()
                    ->publish()
                    ->paginate(10);
            Cache::tags(['tag'])->forever('gettagRecords', $response);
        }
        return $response;
    }
    /**
    ...
    public static function getRecordforEmailById($id) {
        ...
        $response = Cache::tags(['tag'])->get('getRecordforEmailById_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields)
                    ...
                    ->CheckRecordId($id)
                    ->first();
            Cache::tags(['tag'])->forever('getRecordforEmailById_' . $id, $response);
        }
        return $response;
    }
    public static function getFrontList() {
        ...
        $response = Cache::tags(['tag'])->get('tagFrontList');
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->where('chrMain', 'Y')
                    ->get();
            Cache::tags(['tag'])->forever('tagFrontList', $response);
        }
        return $response;
    }
    public static function getFrontListForFooter() {
        ...
        $response = Cache::tags(['tag'])->get('tagFrontListForFooter');
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->where('chrMain', 'Y')
                    ->get();
            Cache::tags(['tag'])->forever('tagFrontListForFooter', $response);
        }
        return $response;
    }
