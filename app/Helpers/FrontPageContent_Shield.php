...
class FrontPageContent_Shield
{
    ...
    public static $newsUri;
    public static function renderBuilder($data)
    {
        $response = '';
        Self::$newsUri = \App\Helpers\MyLibrary::getFront_Uri('news');
            ...
            if (is_array($data)) {
                ...
                foreach ($data as $section) {
                    ...
                    if ($module == 'only_title') {
                        ...
                        $extclass = $section['val']['extclass'];
                        $headingtype = isset($section['val']['headingtype']) ? $section['val']['headingtype'] : "";

                        $response .= Self::OnlyTitleHTML($content, $extclass,$headingtype);
                        ...
                        $sector_slug = '';
                         if (isset($section['val']['sector']) && !empty($section['val']['sector'])) {
                            $sector_slug = $section['val']['sector'];
                        } 
                        ...
                        $response .= Self::interconnectionsHTML($title, $parentorg, $orgclass, $filter, $dbFilter,$sector_slug);
                    ...
                    }else if ($module == 'licence-register') {
                        $title = $section['val']['title'];
                        ...
                        $recIds = array_column($records, 'id');
                        $fill = \Powerpanel\LicenceRegister\Models\LicenceRegister::getBuilderBoard($recIds);
                        ...
                        if ($fill->count() > 0) {
                            ...
                            if ($module != 'licence-register') {

                                $moduleJS = 'assets/js/licence-register.js';
                        ...
                        $response .= Self::licenceregisterHTML($title, $class, $records, $filter);
...
//                        $layout = $section['val']['layout'];
                        ...
                        $response .= Self::AllteamHTML($title, $filter, $dbFilter);
                        ...
                        $layout = $section['val']['layout'];
                         ...
                         if (isset($section['val']['limit']) && !empty($section['val']['limit'])) {
                        ...
                        $response .= Self::allBoardOfDirectorsHTML($title, $config, $layout, $class, $filter,$limit, $dbFilter);
                        ...
                        $response .= Self::allComplaintServicesHTML($title, $config, $layout, $class, $desc, $filter, $dbFilter,$limit);
                        ...
                        $response .= Self::eventHTML($title, $records, $filter, $extraclass, $today);
                        ...
                        $response .= Self::AlleventsHTML($title, $limit, $filter, $class, $eventscat, $today, $dbFilter);

                    } else if ($module == 'career_template') {
                        ...
                        $response .= Self::allCareers($title, $limit, $desc, $filter, $class, $dbFilter);
                        ...
                        $recIds = array_column($records, 'id');
                        $fields = Self::selectFields($config);
                        $fill = \Powerpanel\Service\Models\Service::getBuilderServices($fields, $recIds);
                        if ($fill->count() > 0) {
                            $owlCaurosolLib = 'assets/libraries/owl.carousel/js/owl.carousel.min.js';
                        ...
                        if (isset($section['val']['servicescat']) && $section['val']['servicescat'] != '') {
                            $servicescat = $section['val']['servicescat'];
                        } else {
                            $servicescat = '';
                        }

                        $response .= Self::AllservicesHTML($title, $config, $layout, $filter, $class, $desc, $limit,$servicescat);
                        ...
                        $response .= Self::newsHTML($title, $records, $filter, $extraclass);
                    } else if ($module == 'news_template') {
                        
                        $title = $section['val']['title'];
                        //                        $limit = $section['val']['limit'];
                       $limit = isset($section['val']['limit']) ? $section['val']['limit'] : 12;
                        ...
                        $response .= Self::AllnewsHTML($title, $limit, $filter, $class, $sdate, $edate, $newscat, $dbFilter);
                        ...
                        if (isset($section['val']['filter']) && !empty($section['val']['filter'])) {
                            $dbFilter = $section['val']['filter'];
                            // dd($section['val']['filter']);
                        } else {
                            $dbFilter = false;
                        }
                        $response .= Self::allRegisterApplicationsHTML($title, $limit, $dbFilter);
                        ...
                        if (isset($section['val']['filter']) && !empty($section['val']['filter'])) {
                          ...
                          if (isset($section['val']['sector']) && !empty($section['val']['sector'])) {
                            $sectorfield = $section['val']['sector'];
                        } 

                        $response .= Self::allLicenseRegisterHTML($title, $limit, $filter, $dbFilter,$sectorfield);
                        ...
                        $response .= Self::AllConsultationsHTML($title, $limit, $desc, $config, $layout, $filter, $class, $sdate, $edate, $blogscat, $dbFilter,$sector_consultation);
...
//                            $publicationscat = $section['val']['publicationscat'];
    ...
    public static function eventHTML($title, $records, $filter, $extraclass, $today)
    {
        $response = '';
        $today = date('d-m-y H:i:s');
        $recIds = array_column($records, 'id');
    ...
    public static function AlleventsHTML($title, $limit, $filter, $class, $eventscat, $today, $dbFilter = false)
    {
        $response = '';
        $today = date('d-m-y H:i:s');
        $fields = false;
        if (File::exists(base_path() . '/packages/Powerpanel/Events/src/Models/Events.php') != null) {
            ...
            $fill = \Powerpanel\Events\Models\Events::getAllEvents($fields, $limit, $eventscat, $today, $dbFilter);
                    ...
                    foreach ($eventData as $eventKey => $value) {
                        ...
                        if (isset($dbFilter['dateFilter']) && !empty($dbFilter['dateFilter'])) {
                            if ($dbFilter['dateFilter'] == 'today') {
                                ...
                                if ($value->startDate == $saturday || $value->startDate == $sunday) {
                                    $status = true;
                                    goto OuterLoop;
                                }
                            }
                            ...
                            if ($value->startDate >= $today || $today <= $value->endDate) {
                                ...
                                if($filter == "current-month-events") {
                                    if (date('m', strtotime($value->startDate)) == date('m')) {
                                        $status = true;
                                        goto OuterLoop;
                                    }
                                }else{
    ...
    public static function allCareers($title, $limit, $desc, $filter, $class, $dbFilter = false)
    {
        $response = false;
        if (File::exists(base_path() . '/packages/Powerpanel/Careers/src/Models/Careers.php')) {
            $fill = \Powerpanel\Careers\Models\Careers::getFrontList($limit, $dbFilter);
            ...
            $data = [
                ...
                'filter' => $filter,
                'class' => $class,
                'moduelFrontPageUrl' => $moduelFrontPageUrl
            ];
            $response = view('careers::frontview.builder-section.career', compact('data'))->render();
        }
        return $response;
    }
    ...
    public static function newsHTML($title, $records, $filter, $extraclass)
    {
        $response = '';
        $recIds = array_column($records, 'id');
    ...
    public static function AllnewsHTML($title, $limit, $filter, $class, $sdate, $edate, $newscat, $dbFilter = false)
    {
        $response = '';

        if (File::exists(base_path() . '/packages/Powerpanel/News/src/Models/News.php') != null) {
            $fill = \Powerpanel\News\Models\News::getAllNews($limit, $sdate, $edate, $newscat, $dbFilter);
            if (!empty($fill)) {
                $data = [
                    'title' => $title,
                    'limit' => $limit,
    ...
    public static function AllCandWServiceHTML($title, $limit =12, $filter, $class, $dbFilter = false)
    {
        $response = '';
...
//        $fields = Self::selectFields($config);
    ...
    public static function AllConsultationsHTML($title, $limit, $desc, $config, $layout, $filter, $class, $sdate, $edate, $blogscat, $dbFilter = false,$sector_consultation)
    {
        $response = '';

        $fields = Self::selectFields($config);
        if (File::exists(base_path() . '/packages/Powerpanel/Consultations/src/Models/Consultations.php') != null) {
            $fill = \Powerpanel\Consultations\Models\Consultations::getAllConsultations($fields, $limit, $sdate, $edate, $blogscat, $dbFilter,$sector_consultation);
            if (!empty($fill)) {
                $data = [
                    'title' => $title,
                    'limit' => $limit,
    ...
    public static function allRegisterApplicationsHTML($title, $limit, $dbFilter = false)
    {
        $response = '';
    ...
    public static function allLicenseRegisterHTML($title, $limit, $filter, $dbFilter = false,$sectorfield)
    {
        $response = '';

        $fields = false;
        if (File::exists(base_path() . '/packages/Powerpanel/LicenceRegister/src/Models/LicenceRegister.php') != null) {
            $fill = \Powerpanel\LicenceRegister\Models\LicenceRegister::getAllLicenseRegister($fields, $limit, $dbFilter,$sectorfield);
            if (!empty($fill)) {
                $data = [
                    'title' => $title,
                    'limit' => $limit,
    ...
    public static function interconnectionsHTML($title, $parentorg, $orgclass, $filter, $dbFilter = false,$sector_slug)
    {
        $response = '';
        ...
        if (File::exists(base_path() . '/packages/Powerpanel/Interconnections/src/Models/Interconnections.php') != null) {
            $interconnectionData = \Powerpanel\Interconnections\Models\Interconnections::getBuilderInterconnections($parentorg, $dbFilter,$sector_slug);
            $interdata = array();
    ...
    public static function AllteamHTML($title,  $filter, $dbFilter)
    {
        ...
        if (File::exists(base_path() . '/packages/Powerpanel/Team/src/Models/Team.php') != null) {
            ...
            $fillBoard = \Powerpanel\BoardOfDirectors\Models\BoardOfDirectors::getTemplateTeamList($dbFilter,false);
     ...
     public static function licenceregisterHTML($title, $class, $records, $filter)
    {
        $response = '';
        $response = '';
        $recIds = array_column($records, 'id');
    ...
    public static function allBoardOfDirectorsHTML($title, $config, $layout, $class, $filter, $limit,$dbFilter)
    {
        $response = '';
        if (File::exists(base_path() . '/packages/Powerpanel/BoardOfDirectors/src/Models/BoardOfDirectors.php')) {
            $fill = \Powerpanel\BoardOfDirectors\Models\BoardOfDirectors::getTemplateTeamList($dbFilter,$limit);
            if (!empty($fill)) {
                $data = [
                    'title' => $title,
                    'config' => $config,
    ...
    public static function allComplaintServicesHTML($title, $config, $layout, $class, $desc, $filter, $dbFilter,$limit)
    {
        $response = '';
        if (File::exists(base_path() . '/packages/Powerpanel/ComplaintServices/src/Models/ComplaintServices.php')) {
            $fill = \Powerpanel\ComplaintServices\Models\ComplaintServices::getTemplateComplaintServicesList($dbFilter,$limit);
    ...
    public static function AllservicesHTML($title, $config, $layout, $filter, $class, $desc, $limit,$servicescat)
    {

        $fields = Self::selectFields($config);

        if (File::exists(base_path() . '/packages/Powerpanel/Service/src/Models/Service.php') != null) {
            $fill = \Powerpanel\Service\Models\Service::getAllServices($fields, $limit, $servicescat);
    ...
    public static function serviceHTML($title, $desc, $config, $layout, $records, $filter, $extraclass)
    {
        ...
        $fields = Self::selectFields($config);
        if (File::exists(base_path() . '/packages/Powerpanel/Service/src/Models/Service.php') != null) {
            $fill = \Powerpanel\Service\Models\Service::getServiceList($fields, $recIds, 5);
            foreach ($records as $key => $row) {
                if (count(array_unique($row['custom_fields'])) > 1) {
                    $fill[($key - 1)]->custom = $row['custom_fields'];
                }
    ...
    public static function OnlyTitleHTML($content, $extclass,$headingtype)
    {
        $response = '';
        $data = [
            'title' => $content,
            'extra_class' => $extclass,
            'headingtype' => isset($headingtype) ? $headingtype : ''
        ];
