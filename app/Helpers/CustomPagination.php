...
class CustomPagination {
	...
	*/
	function createLinks(){
			...
			if ($numPages == 1){
					if ($this->showCount){
							$info = '<span class="ac_page_num"> Page ' . $this->currentPage . ' of '. $numPages .'</span>' ;
							return $info;
					}else{
							return '';
					}
...
*/			
			...
			if($this->currentPage > $this->numLinks){
					...
					$output .= '<'.$this->firstTagOpen.' class="pagination_first">'.'<a href="'.$firstPageURL.'"><i class="fi flaticon-right"></i> '.$this->firstLink.'</a>'.'</'.$this->firstTagClose.'>';
			}
			...
			for($loop = $start -1; $loop <= $end; $loop++){
					...
					if($i >= 1){
							if($this->currentPage == $loop){
									$output .= '<'.$this->curTagOpen.' class="active">'.'<a href="javascript:void(0);">' .$loop.'</a>'.'</'.$this->curTagClose.'>';
							}else{
			...
			if(($this->currentPage + $this->numLinks) < $numPages){
					$i = $numPages;
					$output .= '<'.$this->lastTagOpen.' class="pagination_last" >'.'<a href="'.$this->baseURL.$i.'">'.$this->lastLink.'<i class="fi flaticon-right"></i>'.'</a>'.'</'.$this->lastTagClose.'>';
			}
