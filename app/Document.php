...
class Document extends Model {
    ...
    public static function getDistinctFolderDocuments($limit, $page, $position = 0, $filter = false) {
        $response = false;
        $documentFields = ['id', 'txtDocumentName', 'txtSrcDocumentName', 'varDocumentExtension', 'intMobileViewCount', 'intDesktopViewCount', 'intMobileDownloadCount', 'intDesktopDownloadCount', 'chrPublish', 'varfolder', 'fk_folder'];
        $response = Self::select($documentFields)
                ->publish()
    ...
    public static function getRecordById($id) {
      ...
      public static function getDocDataById($ids = false) {

        $response = false;
        $moduleFields = ['documents.id', 'documents.fkIntUserId', 'documents.txtDocumentName', 'documents.txtSrcDocumentName', 'documents.varDocumentExtension', 'documents.chrIsUserUploaded', 'documents.fk_folder', 'folder.foldername'];
        $response = Self::select($moduleFields)
                        ->leftJoin('folder', 'folder.id','documents.fk_folder')
                        ->deleted()
                        ->where('documents.id', $ids)
                        ->get();
     ...
     public static function getDocDataByIds($ids = false) { 
        ...
        $query = Self::select($moduleFields)
                        ...
                        if(!empty($ids) && isset($ids[0]) && !empty($ids[0])) {
                            $query->orderByRaw(DB::raw("FIELD(nq_documents.id, ".implode(',', $ids)." )"));
                        }
    ...
    public static function getFolderInfo($name) {
        $folderdata = DB::table('folder')
                ->select('*')
