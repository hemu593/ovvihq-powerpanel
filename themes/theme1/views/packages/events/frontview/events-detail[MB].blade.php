                ...
                <div class="col-md-12 col-md-12 col-xs-12 animated fadeInUp">
                    <div class="right_content">
                        <div class="back_div">
                            ...
                            $backpageUrl = App\Helpers\MyLibrary::getFront_Uri('events')['uri'];
                            }
                        ...
                        @php
                        $eventstartDate = date('l F jS, Y',strtotime($events->dtDateTime));
                        $eventDisplayDate = $eventstartDate;
                        if(!empty($events->dtEndDateTime) && $events->dtEndDateTime != null){
                        $eventExpityDate = date('l F jS, Y',strtotime($events->dtEndDateTime));
                        $eventDisplayDate = $eventstartDate." to ".$eventExpityDate;
                        }
                        @endphp
                            @if(isset($events->fkIntImgId) && $events->fkIntImgId != '')
								@php $itemImg = App\Helpers\resize_image::resize($events->fkIntImgId) @endphp
								<img src="{{ $itemImg }}" alt="{{ $events->varTitle }}">
								<br/><br/>
							@endif
                        <h1 class="cms_detail_h2 ac-mb-xs-15">{{ $events->varTitle }}</h1>
                        <h6 class="ac-mb-xs-15">{{ $eventDisplayDate }}</h6>
                        @if(isset($events->txtDescription) && !empty($events->txtDescription))
                        <h6>Description</h6>
                        {!! htmlspecialchars_decode($txtDescription) !!}
                        @endif

                        @if(isset($events->varEventLocation) && !empty($events->varEventLocation))
                        <h6>Event Location</h6>
                        {!! htmlspecialchars_decode($varEventLocation) !!}
                        @endif

                        @if(!empty($events->fkIntDocId))
                        @php
