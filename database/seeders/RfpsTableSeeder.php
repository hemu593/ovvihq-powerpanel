...
class RfpsTableSeeder extends Seeder
{
    public function run()
    {
        $pageModuleCode = DB::table('module')->select('id')->where('varTitle', 'Rfps')->first();
        ...
        if (!isset($pageModuleCode->id) || empty($pageModuleCode->id)) {
            if (\Schema::hasColumn('module', 'intFkGroupCode')) {
                DB::table('module')->insert([
                    ...
                    'varTableName' => 'rfps',
                    'varModelName' => 'Rfps',
                    'varModuleClass' => 'RfpsController',
                    'varModuleNameSpace' => 'Powerpanel\Rfps\\',
                    'decVersion' => 1.0,
                    'intDisplayOrder' => 0,
                    'chrIsFront' => 'Y',
                    'chrIsPowerpanel' => 'Y',
            ...
            $pageModuleCode = DB::table('module')->where('varTitle', 'Rfps')->first();
            $permissions = [];
        ...
        $exists = DB::table('cms_page')->select('id')->where('varTitle', htmlspecialchars_decode('Rfps'))->first();

        if (!isset($exists->id)) {
            if (\Schema::hasColumn('cms_page', 'chrMain')) {
                DB::table('cms_page')->insert([
                    'varTitle' => htmlspecialchars_decode('Rfps'),
                    'intAliasId' => MyLibrary::insertAlias(slug::create_slug(htmlspecialchars_decode('Rfps')), $cmsModuleCode->id),
